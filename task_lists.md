...
| 項目 | 内容 |
|---|---|
| 直前に完了 | T-26 の zip と README（目視 2026-09-06 通過）。残りは実運用 1 週間だけ |
| いまここ | **T-29 文言の見直し**と **T-30 エクスプローラーで開く**（どちらも目視待ち）|
| 未解決の判断事項 | なし |

**残るは T-29 / T-30 の目視 ＋ T-26 の実運用 1 週間。**
T-26 の作業ぶん（zip・README）は終わっていて、**日数が経つのを待つだけ**
（規約の 3 日基準の例外）。リポジトリ数は 8 個で足りている。
...
| 8 | 9 | T-24 → T-25 → T-28 → **T-26** | いまここ。**T-26 で v1 が終わる**。DoD は実運用 1 週間なのでカレンダー時間で進む |
...
ls gitpeek-*.log | wc -l
grep -ho '\[[^]]*\]' gitpeek-*.log | grep -v '^\[-\]$' | sort -u
```

**2026-09-06 時点で実行済み: 1 日ぶん / 8 リポジトリ。** リポジトリ数は既に足りている
（`drogon` / `dycora` / `linux` / `onyx` / `vite` / `AerialSoldierMaker` /
`gitviewer` / `save_temperature`）。**残るは日数だけ。**
...
## - [ ] T-29 [Phase 9] 画面の文言を日本語として読めるようにする

**目的**: 「AI が書いた文章」に見える硬さを取る。**利用者の要望（2026-09-06）。**

**参照**: CLAUDE.md §6

**依存**: なし（T-26 の実運用と並行してよい）
...
| どこ | 件数 | 中身 |
|---|---|---|
| `src/i18n/ja.ts` | 約 400 | ボタン・ヒント・ダイアログ本文 |
| `src-tauri/src/**.rs` | 163（テストを除く）| エラーと失敗の理由。**画面にそのまま出る** |

**中身が悪いわけではない。** 「何が起きるか」で書く方針（CLAUDE.md §6）は効いていて、
意味は通る。**削れば直る**タイプだった。

| 症状 | 例 |
|---|---|
| 文脈で自明な主語を毎回書く | **このリポジトリの**フォルダをエクスプローラーで開きます |
| 「〜します」で全文が終わり、リズムが単調 | — |
| 同じ意味を重ねる | 1 つずつ**順に** ／ 取ってきます（**取ってくるだけ**で…）|
| 翻訳調の連体修飾 | リポジトリの中に**置かれた**観点 |
| 硬い名詞 ＋「の可能性があります」 | git プロセスが**動作中の可能性があります** |
| 「〜してください」の多用 | 20 か所 |
| 受け身の空状態 | リポジトリが登録されて**いません** |

**方針（2026-09-06 に利用者が「その方針で」と決めた）**

- **である調に寄せず、丁寧語のまま短く削る**
- ボタンのヒントは**体言止めを使ってよい**
- **git のコマンド名（`fetch` / `clone` / `bare`）は英語のまま残す。**
  CLAUDE.md §6 の「用語をそのまま出さない」は「FF マージ」のような**説明語**の話。
  **診断に要る語（`choices` / `fast-forward` / `base URL`）も残す** —
  接続先の設定は技術的な作業で、周りの文言も同じ語を出している

**実装内容**

1. `src/i18n/ja.ts` を通す（**キーは変えない**。値だけ）
2. `src-tauri/src/**.rs` の**表に出る文字列**を通す（テストの assert 文言は対象外）
3. **文言を直接見ているテストは、意図を見る形へ直す** — 下記

**分かったこと: 文言を直接見るテストは、文章を古いまま固定する**

`clone.rs` / `review.rs` / `writeops.rs` の 4 本が、`contains("既にあります")` の
ように**言い回しそのもの**を見ていた。**直すたびにテストが落ちるので、直すのが
億劫になって古い文章が残る。** 意図のほうを見る形へ直した
（どのフォルダで止まったか ＝ フォルダ名 ／ 打つ手が書いてあるか ＝「切り替え」）。

**1 件は文言のほうが間違っていた。** `explain_merge` から `fast-forward` を
落としたらテストが落ちた。UI は「早送り（fast-forward）」と併記する方針なので、
**Rust だけ落としたのが不統一**だった。テストではなく文言を直した。

**受け入れ条件**

- ▸コマンド: `npm run test:rust` / `npm run check:rust` / `npm run test` / `npm run typecheck`
- ▸コマンド: `ja.ts` の**キーが 1 つも増減していない**こと（値だけの変更）
- ▸目視: 画面を一通り触って、**読んで引っかからない**こと
- ▸目視: **パスを含む文言のバックスラッシュが 1 段はがれていない**こと
  （`.gitpeek\skills\`。T-25 で踏んだ）

**⚠️ このタスクはエージェントが単独で完了を宣言してはいけない。**

**非スコープ**: 英語 UI ／ i18n ライブラリの導入（CLAUDE.md §6）／
`docs` と `CLAUDE.md` の地の文（画面に出ない）

---

## - [ ] T-30 [Phase 9] リポジトリの右クリックに「エクスプローラーで開く」

**目的**: 登録したリポジトリのフォルダをエクスプローラーで開けるようにする。
**利用者の要望（2026-09-06）。**
...
- Rust に `open_repository_folder(repository_id)` を足す。
  **受け取るのは登録の ID だけで、パスはフロントから渡させない** —
  `open_log_folder` に「capability は広げない。画面側から任意のパスを開ける形に
  しない」と書いてあるので、同じ形に揃えた。パスは `Store::repository()` が引く
- **開く前に実在を確かめる。** 登録したあとで移動・削除されたリポジトリはよくある
- 出し分けは純関数（`lib/repositoryMenu.ts`）。**押せないときも消さず、理由を出す**
  （CLAUDE.md §6）。ついでに `hasRemotes` も同じところへ移した —
  `.tsx` に直書きされていて**テストが 1 つも当たっていなかった**

**受け入れ条件**

- ▸コマンド: `npm run test` — 押せる / フォルダが無い / 登録が無い / 一覧が空
- ▸コマンド: `npm run test:rust` / `npm run check:rust` / `npm run typecheck`
- ▸目視: 右クリック →「エクスプローラーで開く」でそのフォルダが開くこと
- ▸目視: **フォルダを消した登録では押せず、理由が読めること**（消えていないこと）

**⚠️ このタスクはエージェントが単独で完了を宣言してはいけない。**

**非スコープ**: ファイル単位で開く／ターミナルで開く／エディタで開く
...
| T-28 | 9 | `Givsoner` → `GitPeek` の改名（**identifier ごと変え、移行はしない**。経緯は DESIGN.md §2.3.1） | `0495f94` |
