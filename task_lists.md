...
| 次にやる | **T-10 ブランチ / タグツリー UI — 実装は終わり、目視確認待ち** |
| 未解決の判断事項 | なし |

**T-10 は ▸コマンド の受け入れ条件を満たしたところで止めてある。** ▸目視 が 4 項目
...
| ~~新規~~ | `src/components/common/ContextMenu.tsx` | T-03 で作成済み。そのまま使う |
...
**T-10 で決めたこと（DESIGN.md §4.4, §6.4 に転記済み）**

- **タグにはチェックボックスを置かない。** 外してもグラフが変わらないので、
  「押せるが何も起きない」より最初から無いほうがよい。`excluded` にタグは入らない。
- **ahead/behind は 0 のとき何も出さない。** 上流の無いブランチも空欄にする。
  同期しているのか上流が無いのかはツールチップで見せる。
- **開閉状態は「畳んだ ID」を保存する**（`state.json` の `collapsedTreeNodes`）。
  展開側だと「全部畳んだ」と初期状態を区別できない。既定値は `["tag"]`。
  **`expandedTreeNodes` から名前を変えた**ので、Rust 側の `Default` も手書きにしてある。
- グラフから外した ref はツリー上で淡色にする（チェックだけでは遠目に読めない）。

**T-10 で見つけて直した不具合**

**ref が数千本あるとリポジトリ一覧が潰れた（目視 1 巡目）。** ツリー側のスクロールペインを
`flex: 1 1 auto` にしていたため、中身の高さ（数万 px）が基準サイズになり、縮小が
基準サイズの比で配分されて**リポジトリ一覧が 9px** になった（vite / onyx で発生）。
基準を 0 にして、上段の「中身なり・最大 50%」が先に決まるようにした。

**開閉の三角が小さすぎた（目視 1 巡目）。** `▸` / `▾` の 10px 文字ではどちらの状態か
読めなかった。ボーダーで 7×10px の三角を描き、開いているときだけ 90 度回す形に変えた。


**コミットリストが `layout.rows` ではなくコミット配列を並べていた。** 可視 ref を絞ると
グラフの行だけが減り、行の文字と線が 1 行ずつずれていた（date 表示でも同じ症状が出る）。
T-09 の絞り込みを誰も使っていなかったので露見していなかった。`CommitList` が `rows` を
順に辿って SHA でコミットを引くよう直し、CLAUDE.md §6 と DESIGN.md §6.3 に不変条件として
書いた。件数表示・SHA ジャンプ・キーボード移動・スクロールバーの ref マーカーも
絞り込み後の集合を見る。
