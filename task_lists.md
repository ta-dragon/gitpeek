...
| 項目 | 内容 |
|---|---|
| 直前に完了 | GitLab への公開と v0.1.0 のリリース ／ アイコンの差し替え ／ README と CLAUDE.md の書き直し（**いずれも T-26 の外**。下記）|
| いまここ | **T-31（取ってきて取り込む）の目視待ち** ＋ **T-26 の実運用**（2026-09-08 時点で 3 日ぶん / 8 リポジトリ。あと 4 日）|
...
**次のリリースで直ること**: v0.1.0 の zip は `1aadbf5` から作ったので、**中の exe のアイコンは
古い Tauri のロゴのまま**。新しいアイコンは v0.1.1 以降に入る。
...
| 9 | 9 | **T-31** | 2026-09-08 に利用者が決めて足した（`git pull --ff-only` 相当）。T-17 / T-18 の上に乗るだけで、T-26 の待ち時間とは独立 |
...
**2026-09-08 時点で実行済み: 3 日ぶん（09-06 / 09-07 / 09-08）/ 8 リポジトリ。**
