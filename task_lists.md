...
| 未解決の判断事項 | なし（T-12 で 1 件持ち越し。下記） |

**EUC-JP の判別は当たらないことがある。** ひらがなは 1 バイトずつが Shift_JIS の半角カナと
して妥当なので、**厳密デコードが通って Shift_JIS と判定される**（置換文字も出ないので
画面が化けるまで気付けない）。順序で判別する方式の帰結であり、直す手段は手動上書き。
`encoding.rs` の `euc_jp_hiragana_is_misdetected_as_shift_jis` が挙動を固定している。
**EUC-JP のリポジトリを常用することになったら v1.1 で `chardetng` を再検討する**
（DESIGN.md §9.1）。
...
| T-12 | 4 | 文字コード自動判別と改行コード検出 | (このコミット) |
