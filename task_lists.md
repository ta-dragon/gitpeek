...
| T-37 | v1.1 | ブランチが相手に取り込まれているかの判定（`merge-tree` を**一時フォルダへ隔離**。**CLAUDE.md §1 を緩める 4 件目**。squash / 途中まで / squash 後に書き換え を ③ と ② で見分ける。**git の下限を 2.38 へ**。テストを見直した際、**相手が隣の行を書き換えると誤って「途中まで」と答える**不具合を囮のテストで見つけ、②を二分探索より先にした（DESIGN.md §17.1）。▸目視なし — 画面は T-38。DESIGN.md §7.6）| （このコミット）|
