...
5. **骨子のままのタスクは、着手前に詳細化する。** ⑥実装内容と⑧受け入れ条件が埋まって
   ...
   未着手は **T-37 / T-38 / T-39**（ブランチが取り込まれているか）。**T-37 は詳細化済み、
   T-38 / T-39 は骨子**なので、着手するときに T-37 の実物と突き合わせて詳細化する（2026-09-18 時点）。
...
2. **T-37 に着手する**（「v1.1 — ブランチが取り込まれているか」の節）。2026-09-18 に利用者が決めた。
...
| いまここ | **T-37 未着手**（ブランチが取り込まれているか。T-37 → T-38 → T-39）|
...
**T-37 → T-38 → T-39 の順**に着手する。T-38 まで終われば、ローカルブランチの取り違えは起きなくなる。
...
1. **`git merge-tree --write-tree`（③）を使ってよい。書き込みは一時フォルダに限って許可する**
   ...
   **CLAUDE.md §1 を緩める 4 件目**で、緩め方は既存の 3 件と揃える（表に 1 行足す）
2. **git は 2.38 以上を必須にする**（`merge-tree --write-tree` が 2.38 から）。**対応外の git では警告して止める**
   ...
   下限を 2.20 → 2.38 に上げる）。**②だけで判定する逃げ道は作らない**
...
**着手前に確かめたこと（2026-09-18。git 2.43.0.windows.1。scratchpad の捨てるリポジトリで再現）**

| 状況 | ③ 仮にマージして相手と同じ中身か | ② 差分が squash コミットと一致するか |
|---|---|---|
| squash の後、main は別のファイルだけ進んだ | 一致 | 一致 |
| squash の後、**ブランチに 1 つ足した** | 不一致 → **1 つ手前までなら一致** | — |
| squash の後、**main で同じ行をさらに書き換えた** | 不一致（衝突。終了コード 1）| 一致 |

- **一時フォルダへ逸らすとリポジトリは変わらない**: 新しい tree ができる場面で、隔離先に 1 ファイル、
  リポジトリの `count-objects` は 24 → 24
- **速さ**: vite で 1 回 0.12 秒
- `merge-tree --write-tree` の終了コードは **0 ＝ 衝突なし / 1 ＝ 衝突あり**。1 行目が結果の tree
- 数字と表の行き先は **DESIGN.md §7.6（T-37 で新設）**。ここには残さない

## - [ ] T-37 [v1.1] ブランチが相手に取り込まれているかの判定（Rust）

**目的**: 1 本のブランチと 1 つの相手について、「普通にマージ済み / 中身は入っている（squash なら
どのコミットか）/ 途中まで入っている / squash 後に書き換えられた / 入っていない」を判定して返す。
**画面は T-38。** ここでは判定とコマンドまで。

**参照**: DESIGN.md §7.6（新設）/ §3.1（exec）/ §4.2（起点 ref）/ §4.5（メモリ上の祖先関係）/
CLAUDE.md §1, §2, §8

**依存**: なし

**着手時に確認した現況（規約 4。2026-09-18 に調べた）**

- git の入口は `exec.rs` の `run` / `run_streaming` / `run_progress` で、**3 つとも `build()` を通る**。
  `build()` が付ける環境変数は `GIT_TERMINAL_PROMPT` と `GIT_SSH_COMMAND` の 2 つだけで、
  **呼び出しごとに環境変数を足す口は無い**
- 祖先関係はメモリ上で引ける（`graph/reach.rs` の `CommitIndex`。`reachable_from` / `ahead_behind`）
- 既定ブランチは `RepositorySnapshot.default_branch`、コミット時刻は `CommitMeta.commit_time`
- 版の下限は `git/detect.rs` の `MIN_MAJOR` / `MIN_MINOR`（いま 2.20）。**下回ると起動画面から先へ
  進めない**（`isGitUsable`）。**この作りのまま下限を 2.38 に上げる**（共通の決めごと 2）
- `tempfile` は dev-dependency にしか無い。**一時フォルダは std だけで作り、依存を足さない**

**判定の順番**

1. **相手の祖先** → 普通にマージ済み（git を呼ばない。`CommitIndex`）
2. **③ 相手とブランチ先端を `merge-tree`** → 結果の tree が相手の tree と同じなら「中身は入っている」。
   続けて②で squash コミットを探し、見つかれば添える
3. ③が不一致なら、**分岐点から先端までの第一親の並びを二分探索**し、どこまでなら一致するかを探す →
   「N コミット中 M 個まで入っている」
4. 一致が 1 つも無ければ②だけで探す → 見つかれば「squash されたが、その後相手で書き換えられている」
5. どれも無ければ「入っていない」

**②の突き合わせ方**

- 比べる相手は、**分岐点より後に相手側へ入ったコミットのうち、ブランチ先端より後の時刻のもの**だけ
  （squash はブランチの最後のコミットより後にしか起きない）
- さらに**触ったファイルの組がブランチと同じもの**に絞る（`git log -z --format=%H --name-only` を 1 回）。
  差分まで比べるのはたいてい 0〜2 件
- 差分は**行番号と `index` 行を落として**突き合わせる。git の `patch-id` は標準入力を読むが、
  **`exec.rs` の標準入力は塞いだままにする**ので、Rust 側で比べる

**実装内容**

1. `git/exec.rs` … **環境変数を足せる口**（`run` に揃えた 1 つ。`build()` を通す）
2. `git/scratch.rs`（新設）… **一時フォルダ**。`%TEMP%` の下に `gitpeek-merge-<一意>` を作り、
   **`Drop` で必ず消す**（失敗しても、中止しても）。起動時に、前回落ちて残った**自分の接頭辞のフォルダだけ**消す
3. `git/contained.rs`（新設）… 判定。種類は列挙で返す（`Ancestor` / `Contained { squash }` /
   `Partial { upto, total, squash }` / `SquashedThenChanged { squash }` / `NotContained`）
4. `lib.rs` … コマンド `check_containment`（リポジトリ ID ／ 調べるブランチ ／ 相手。どれも完全な ref 名）
5. `git/detect.rs` … **下限を 2.38 に上げる。** 起動画面の文言に、なぜ 2.38 が要るか（ブランチが
   取り込まれているかを調べるのに使う）を足す（`i18n/ja.ts`）。README の動作環境も直す
6. `scripts/make-test-repos.sh` … `squash` リポジトリ（上の 3 状況 ＋ 普通のマージ ＋ 未マージ）
7. ドキュメント … DESIGN.md §7.6 を新設（目次・早見表・付録 A も）／ §3.4 の下限と理由 ／
   **CLAUDE.md §1 の表に 4 行目** ／ CLAUDE.md §2 の入口の説明

**制約**

- git の実行は `exec.rs` を通し、**新しい口も `build()` を通す**（CLAUDE.md §2）
- **リポジトリの中へは 1 バイトも書かない。** 書いてよいのは一時フォルダだけ（CLAUDE.md §1 の 4 件目）
- **一時フォルダを OneDrive の下に作らない**（CLAUDE.md §5 と同じ理由）
- フロントからパスや git のフラグを受け取らない。受け取るのは ref 名だけで、**実在する ref か Rust が確かめる**
- **依存を足さない**

**受け入れ条件**

- ▸コマンド: 結合テスト（`squash` リポジトリ）— 判定の 5 種類それぞれに当たる ／ **判定の前後で
  リポジトリの `count-objects` が変わらない** ／ **一時フォルダが残らない**（中止・失敗のときも）
- ▸コマンド: **守りを外すと落ちる**ことを 1 度確かめる（隔離の環境変数を外すと `count-objects` が増える）
- ▸コマンド: 版の判定 — **2.37.x は使えない、2.38.0 は使える**（`detect.rs` の単体テスト）
- ▸コマンド: 依頼と結果の JSON を両方向で固定する
- ▸コマンド: `npm run test:rust` / `npm run check:rust` / `npm run test` / `npm run typecheck`
- ▸目視: なし（画面は T-38）— **実リポジトリでの当たり方と、2.38 未満で止まる画面は T-38 の目視でまとめて見る**

**非スコープ**: 画面（T-38）／ 全ブランチとの比較（T-39）／ GitLab・GitHub の API（外へ通信しない）／
タグを相手にすること（起点 ref ではない）

---

## - [ ] T-38 [v1.1] 取り込まれているかを画面に出す ＋ git の版の警告

**目的**: ローカルブランチが既定の相手に取り込まれているかを自動で調べ、ブランチ一覧とチップに印を
付ける。リモートブランチと、既定以外の相手は右クリックから調べる。git 2.38 未満では警告を出す。
...
- **git の版**: 2.38 未満は起動画面で止まる（T-37 で下限を上げる）。**止まった理由が読めるか**をここで目視する
