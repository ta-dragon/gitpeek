...
| いまここ | **T-23 レビュー UI と結果の永続化。実装済み・目視の途中**（1 巡目で 1 件出て直した）|
| 未解決の判断事項 | なし。**T-23 の目視の残りを消化すること** |

**T-23 はまだ「完了済み」へ畳んでいない。** 規約 3 のとおり、目視項目が残っている
あいだはエージェントが単独で完了を宣言してはいけない。**残りの目視は T-23 の節の
受け入れ条件（▸目視 の行）が正**で、下の「T-23 の目視の進み具合」に消化状況を書いてある。

**T-24 以降が骨子**なので、着手時に詳細化そのものを最初の作業として行うこと（規約 5）。
T-23 が終われば Phase 8 が終わり、そのあと Phase 9（T-24 → T-25 → T-26）。

### T-23 の目視の進み具合（2026-09-06 時点）

| # | 見るもの | 状態 |
|---|---|---|
| 1 | `Ctrl+Shift+A` でドロワーが開き、コミット情報の列が畳まれて差分が潰れない | 未 |
| 2 | 実行前パネル: 送らないファイルと理由 / 「分けて送ります」/ 押せない理由 | 未 |
| 3 | 実行中に**本文が少しずつ流れる** | **1 巡目で不合格 → `a952b6b` で修正。再確認待ち** |
| 3' | 中止が効く | **通過** |
| 4 | 指摘のバッジが該当行に出る / 当たらなかった指摘がドロワーに残る | 未 |
| 5 | 同じ差分を 2 回で履歴が 2 件になり、開くと当時の結果が出る | 未 |
| 6 | 構造化に失敗したとき Markdown が出て理由が添う | 未 |
| 7 | 「Markdown で書き出し」で読めるファイルができる | 未 |

**3 の直しは、ホールドバックの幅（256 → 64 文字）** が原因だった。詳細は
DESIGN.md §10.7.1 と下の「落とし穴」。**再確認では、短いレビュー結果でも
文字が流れていくことを見ること。**
...
- 型は `src/lib/ipc.ts` に揃えてある。判定と整形の純関数は
  `src/lib/reviewPlan.ts` / `reviewFindings.ts` / `reviewMarkdown.ts`
...
T-20 は 3 巡・3 件、T-21 は 2 巡・2 件、**T-23 は 1 巡目で 1 件（途中）**。
...
- **指摘から差分へ「飛ぶ」のは、ファイルを開くところまで**（T-23）。
  **行までスクロールしない。** ドロワーをコミット情報の列と入れ替える判断の
  根拠に「指摘から差分へ飛べる」と書いてあるので、ここは弱いまま。
  足すなら `DiffBody` の仮想スクローラへ行番号を渡す（20 行ほど）
...
| 変更 | `src-tauri/src/lib.rs` | `start_review` が保存して `StoredReview` を返す ／ `list_reviews` / `load_review` / `export_markdown` / `set_repository_llm_profile` |
| 変更 | `src-tauri/src/store/mod.rs` | リポジトリの既定プロファイルの保存 |
| 変更 | `src-tauri/capabilities/default.json` | `dialog:allow-save`（Markdown の保存先を選ぶ）|
| ~~新規~~ | ~~`src-tauri/tests/reviews.rs`~~ | **作らなかった。** `store/reviews.rs` の単体テストが tempdir で実ファイルを読み書きしており、別ファイルにすると同じことが 2 か所になる。走らせる経路まで含む 3 本は `tests/review.rs` に足した |
...
- `export_markdown(path, text)` … Markdown を書く。**`@tauri-apps/plugin-fs` は入れない** —
  依存 2 つと引き換えに得られるのは「任意のファイルを書く」機能で、要るのは 1 用途だけ。
  行き先は保存ダイアログで選んだパスに限る
...
- ▸目視: 実行中に**本文が少しずつ流れる**こと ／ **中止が効く**こと
  （**1 巡目で前者が不合格**。ホールドバックの幅が応答より広く、最後にまとめて
  1 回だけ届いていた。`a952b6b` で 256 → 64 文字。**短い結果で再確認すること**）
