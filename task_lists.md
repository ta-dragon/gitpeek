...
| 次にやる | **T-08 判定ゲート — 美しさの検証と調整**（調整は済み。**合否はユーザーの目視待ち**） |
| 同時に目視待ち | T-27 orphan ブランチと履歴の終端の印（T-08 の目視から出た要望を切り出したもの） |
| 未解決の判断事項 | なし（判定用リポジトリは 7 個そろった） |
...
**判定用リポジトリは 7 個そろった**（drogon 2,298 / vite 10,186 が追加された）。内訳は T-08 本文。
...
**着手時の突き合わせ（2026-09-03）**

- **判定用リポジトリ（2026-09-03 に vite と drogon が追加され、7 個になった）**
  ...
  | onyx | 20,285 | 52 | 11.43 | 4 |
  | linux | 148 万 | — | — | DESIGN.md §4.1 の非対象 |

  小さい 3 つはマージが 1 件も無い一直線なので、枝分かれの判定には使えない。
- **調整して測った結果は DESIGN.md §15.1 に書いた。** 数字はそちらが正。
  ここに残すのは結論だけ。
  - 第 2 親の合流案は**不採用**（効果 max_lane 49→45 に対し、合流 179 本中 128 本が左向き・
    最大 39 レーン横断）。CLAUDE.md §3-3 は変更しない
  - `RESERVE_ROWS` は **2 のまま**（1〜4 のどれでも onyx の max_lane と平均レーン数が
    変わらなかった）。「線が別のブランチに化ける」と感じたら 3〜4 へ上げる余地がある
  - **レーン色の並び順を入れ替えた**（olive と brown が隣同士で ΔE 25.7 しか無く
    見分けられなかった）
  - **目視の指摘 2 巡目でライトのレーン色を明るくし直した。** 元のライトは白との輝度比が
    平均 5.25（ダークは 6.75 だが背景が暗いので体感は逆）で、グラフ全体が暗く沈んでいた。
    色相はそのままに L\* 平均 45.8 → 56.7 / 輝度比 平均 5.25 → 3.55 へ。
    **幹の色は据え置き**（ここだけ暗いままにして幹を主張させる — DESIGN.md §5.2）
  - 幹は onyx 4,000 行で一直線（lane 0 に出入りする辺 0 本）
- **目視の指摘 3 巡目: 「グラフが途中で切れている」（onyx `68912649c4` と `3da8eae7a0` の間）。**
  原因は描画ではなくレーン割り当てだった。1 つ上の `e53fafa97c` が **ルートコミット**
  （orphan ブランチ `origin/assets/...` の起点、親なし）で lane 1 の線がそこで止まり、
  **2 行後に無関係な枝が同じ lane 1 を使っていた**。色はレーン単位なので、
  1 本の線が穴を空けて続いているように見えていた。
  **ルートで終わったレーンは二度と配らない**ようにして直した（CLAUDE.md §3-2 に追記）。
  費用は onyx で max_lane 49 → 52 のみ（同時レーン数の平均は 11.43 のまま）。
  vite / drogon は変化なし。
- **残る見にくさはレーン数**（onyx は同時レーン平均 11.4 / p99 39）。リモート追跡ブランチ
  1,835 本を全部表示していることの帰結で、レーン割り当てでは減らない。効くのは T-10 の
  可視 ref 絞り込み。**この点を理由に不合格にするなら、直すのは T-08 ではなく T-10。**
...
## - [ ] T-27 [Phase 2] orphan ブランチと履歴の終端に印を付ける

**目的**: 幹と繋がっていない履歴を見分けられるようにする。

**経緯**: T-08 の目視でユーザーから出た要望。T-08 は「新機能を追加しない」と決めてあるので、
ここへ新 ID を発番して切り出した（規約 2）。直前に直した「グラフが切れて見える」
（ルートで終わったレーンの再利用）と同じ原因の、残りの半分にあたる。
**線がそこで止まった理由**が画面に出ていなかった。

**参照**: DESIGN.md §5.2, §6.3 / CLAUDE.md §3-2, §6
...
| 種別 | パス | 内容 |
|---|---|---|
| 新規 | `src-tauri/src/graph/component.rs` | コミット DAG の連結成分（union-find） |
| 変更 | `src-tauri/src/model.rs`, `git/refs.rs`, `git/snapshot.rs` | `RefEntry.orphan` |
| 変更 | `src/lib/graphPath.ts` + `.test.ts` | `rootCapPath`（終端の横棒） |
| 変更 | `src/components/graph/CommitGraph.tsx`, `styles/graph.css` | 終端の描画 |
| 変更 | `src/components/commits/RefChips.tsx`, `styles/app.css`, `i18n/ja.ts` | チップの印 |
| 変更 | `scripts/make-test-repos.sh`, `tests/snapshot.rs`, `tests/lanes.rs` | `orphan` リポジトリ |

**実装内容**

- **グラフ: ルートコミット（親なし）のノードの下に短い横棒を引く。** レーンの終わり方のうち
  マージと畳み込みは線が別レーンへ曲がって続くので終わりが見えるが、ルートだけは線が
  その場で止まる。判定に新しい計算は要らない（`parents.length === 0`）。
  リポジトリ本来の初回コミットにも同じ印が付く（そこも履歴の終端なので正しい）。
- **ref チップ: orphan ブランチのチップを破線の枠にし、`⊥` を前置する。**
  グラフの終端と同じ形を印にして両者を結び付ける。ホバーで説明を出す。
- **orphan の判定は連結成分。** 親子の向きを無視した無向グラフで幹の島を求め、
  そこに入らないコミットを指す ref に印を付ける。到達可能性（T-09）とは別物で、
  union-find 1 回で終わる。**マージで合流した無関係履歴は orphan ではない**
  （繋がっているので線も途切れない）。

**制約**

- 表示文言は `src/i18n/ja.ts`（CLAUDE.md §6）
- 座標の定数は `src/lib/graphPath.ts` に置き、純関数としてテストする（CLAUDE.md §8）

**受け入れ条件**

- ▸コマンド: `cargo test` — 連結成分の単体テストと、`orphan` リポジトリの結合テスト
  （orphan ブランチにだけ印が付く / 合流した無関係履歴には付かない）
- ▸コマンド: `npm run test` — `rootCapPath` が行とレーンからはみ出さない
- ▸コマンド: `npm run typecheck`, `cargo clippy --all-targets -- -D warnings`
- ▸目視: onyx の `origin/assets/...` のチップに印が出る
- ▸目視: 履歴の終端（ルート）に横棒が出て、線が「切れた」ようには見えない
- ▸目視: ライト / ダーク両方で印が読める

**非スコープ**

ブランチツリー側の表示（T-10 で `RefEntry.orphan` をそのまま使う）／
orphan ブランチの絞り込み（T-10 の可視 ref）
