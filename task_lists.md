...
最初の作業として行うこと（規約 5）。**T-26 は T-28 のあと** — 配布する exe の名前が
変わるので、先にリネームを済ませないと受け入れ条件を書き直すことになる。
...
| 8 | 9 | T-24 → T-25 → **T-28** → T-26 | いまここ。**T-28 と T-26 は着手前に詳細化**（規約 5）。T-26 を最後にするのは、配布する exe の名前が T-28 で変わるため |
...
**依存**: T-28（**配布する exe の名前が T-28 で変わる**。先に済ませること）
...
## - [ ] T-28 [Phase 9] Givsoner → GitPeek のリネーム

**目的**: 製品名を `GitPeek` に変える。**利用者が 2026-09-06 に決めた。**

**参照**: CLAUDE.md §5, §10 / DESIGN.md §2.3, §13.4

**依存**: T-25

**着手時に確認した現況（規約 4。2026-09-06 に調べた）**

`Givsoner` は **47 ファイル・約 180 か所**に出る。綴りは 9 種類ある。

| 綴り | 件数 | 何に効くか |
|---|---|---|
| `givsoner_lib` | 53 | Rust の lib クレート名。全結合テストが `use` している |
| `Givsoner` | 47 | 画面の名前・`productName`・ウィンドウタイトル・md の地の文 |
| `givsoner` | 38 | Cargo / npm のパッケージ名・コメント |
| `com.tatsu.givsoner` | 27 | **Tauri の `identifier`。`%APPDATA%` フォルダ名 ＋ keyring のサービス名** |
| `GIVSONER` | 11 | テスト用の環境変数（`GIVSONER_LLM_BASE_URL` / `GIVSONER_SKIP_SYMLINK_TEST`）|
| `givsoner-test-repos` | 4 | `%TEMP%` のフィクスチャ置き場（CLAUDE.md §8）|
| `Givsoner.exe` | 4 | 配布物の名前（T-26 の受け入れ条件） |
| `Givsoner.bat` | 2 | 開発時の起動口（CLAUDE.md §10） |
| `givsoner.exe` | 1 | 同上 |

ほとんどは名前が見えるだけだが、**2 か所は利用者のデータに触る**。
...
- **`%APPDATA%` の中身に手を出すのは、利用者が「引き継ぐ」と決めたときだけ。**
  勝手に消さない・移さない（CLAUDE.md §1 の「黙ってやらない」に倣う）
- 表示文言は `src/i18n/ja.ts`（CLAUDE.md §6）。`index.html` の受け皿だけが例外（§6）
- **まとめて書き換えたあとは `git diff` を全部読む**（T-25 で、隣のコメントを
  巻き込んだ事故があった）

**非スコープ**: アイコンの差し替え／リポジトリ名（`gitviewer`）とリモート URL の変更
...
| T-25 | 9 | 設定画面とショートカット ＋ 差分内検索（目視 9 項目通過・要望 1 件。**依存を増やさず**） | `3af5ea2` `808afb2` |
