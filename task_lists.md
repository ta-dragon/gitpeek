...
| 項目 | 内容 |
|---|---|
| 直前に完了 | T-24 ログファイルとエラー処理の仕上げ（目視 2026-09-06 通過）|
| いまここ | **T-25 設定画面とショートカット。実装とテストは通った。目視待ち**（下の表）|
| 未解決の判断事項 | なし。実装レベルの選択は 2026-09-06 に利用者が決めた（T-25 の節）|

**残るは T-25 → T-26 だけ。** **T-26 はまだ骨子**なので、着手時に詳細化そのものを
最初の作業として行うこと（規約 5）。

### T-25 の目視の進み具合（2026-09-06 時点）

| # | 見るもの | 状態 |
|---|---|---|
| 1 | `Ctrl+,` で設定が開き、`Esc` で閉じる ／ ヘッダの「設定」からも開く | 未 |
| 2 | 8 つのタブが読める ／ **テーマと接続先が設定画面の中にある** | 未 |
| 3 | 値を変えると**即座に反映**され、閉じて開き直しても残る（再起動後も）| 未 |
| 4 | **不正な値（空欄・範囲外・数字でない）で理由が出て、保存されない**こと（欄は消えない）| 未 |
| 5 | git のパスを変えると**その場で確認が走り**、結果が読める（使えないパスも試す）| 未 |
| 6 | **ショートカットの一覧**が出て、DESIGN.md §6.5 の表が全部効く | 未 |
| 7 | `Ctrl+F` で差分の中を探せる（件数 / 前後 / `Esc` で閉じる）／ **畳んである差分では開かない** | 未 |
| 8 | ログのタブから場所が読め、「ログフォルダを開く」で開く | 未 |
| 9 | **リポジトリの右クリック →「このリポジトリの設定」に既定の接続先が出る** ／ 変えると実行前パネルの初期値も変わる | 未 |

**4 と 7 は端を見てください。** 4 は `0` と最大値ちょうど（例: 前後の行数に
`1000000`）、7 は**当たらない言葉**と**同じ行に 2 つある言葉**。

**9 は T-23 の取りこぼし。** 既定の接続先は覚えていたのに画面から読めず、
**勝手に変わっているようにしか見えなかった**（clone の「この保存先を次回から
既定にする」と同じ指摘。CLAUDE.md §6）。**md の見直しで見つけて塞いだ**もので、
実装は T-25 のコミットに入っている。
...
`i18n/ja.ts` の `.gitviewer\skills\` が `\s` になっていて、画面には
...
| 8 | 9 | T-24 → **T-25** → T-26 | いまここ。**T-25 は実装済み・目視待ち**。T-26 は着手前に詳細化（規約 5）|
...
**目的**: 散らばっている設定を 1 画面へ集め、DESIGN.md §6.5 の表を全部効くようにする。

**参照**: DESIGN.md §6.5, §12.2 / CLAUDE.md §6, §8

**依存**: T-24

**着手時に確認した現況（規約 4。2026-09-06）**

- **設定の実体は `settings.json`（`Settings` 構造体）。画面から触れるのは一部だけ。**

  | いま触れるもの | どこから |
  |---|---|
  | テーマ | ヘッダの選択 |
  | 差分のレイアウト / 文脈行 / 空白無視 / 改行の表示 | 差分ツールバー |
  | 接続先（LLM）と観点（グローバル skill）| ヘッダの「AI レビューの接続先」（**既にタブ 2 枚**）|
  | 可視 ref / リポジトリの既定接続先 / リポジトリ内 skill | リポジトリ一覧の右クリック |
  | 並び順（topo / date）| コミット一覧の列ヘッダ |

  **触れないもの**: git のパス（起動時の未検出画面だけ）／ `workspaceRoot`（clone の
  ダイアログだけ）／ `collapseLines` / `collapseBytes` ／ `fetch.staleWarningDays` ／
  `review.concurrency` / `review.contextLines` ／ 日時形式（`dateFormat`）

- **ショートカットは 4 か所に散っている**（`App.tsx` / `hooks/useCommitNavigation.ts` /
  `hooks/useFileNavigation.ts` / `components/diff/useWorkingTree.ts`）。
  **効いていないのは `Ctrl+F` と `Ctrl+,` の 2 つだけ**（DESIGN.md §6.5）
- `LlmProfilesDialog` は既にタブを持つので、**設定画面へタブごと引き取れる**
- **ログの場所は T-24 で取れる**（`log_status` / `open_log_folder`）。設定画面に出すのは T-25
- **`normalize()` が締めているのは theme / dateFormat / diffLayout / commitOrder /
  review.concurrency だけ。** 数値の欄（`contextLines` / `collapse*` /
  `staleWarningDays` / `review.contextLines`）は締めていないので、
  **画面から入る値の検証が要る**（純関数 ＋ Rust の `normalize()` の両方）

**実装レベルの選択（2026-09-06 に利用者が決めた）**

**着手前に 2 つ聞き、どちらも「やる」だった。**

| 決めたこと | 誰が / なぜ |
|---|---|
| **`Ctrl+F`（差分内検索）を T-25 で作る。開いているファイルの中だけ** | **利用者の判断。** v1.1 送りと並べて聞いた結果。T-23 で作った `rowIndexForLine` と飛ぶ仕組みが使えるので安く済む。**当たった行へ移動して囲むだけで、文字は塗らない**（語単位差分と構文色に 3 つ目の色を重ねると読めなくなる。DESIGN.md §6.5.1）|
| **ヘッダのテーマ選択と「接続先」ボタンを設定画面へ畳む** | **利用者の判断。** 入口はヘッダの「設定」と `Ctrl+,` の 2 つだけになった。**リポジトリ 1 つぶんの設定は混ぜない**（右クリックのまま。CLAUDE.md §6）|

| その他に決めたこと | 理由 |
|---|---|
| **設定は 1 枚のダイアログ ＋ タブ**（一般 / 表示 / 差分 / fetch / AI レビュー / 観点 / ショートカット / ログ）| 縦に全部並べると、どこに何があるのか読めない。`LlmProfilesDialog` が既にタブなので形も揃う |
| **入口はヘッダの「設定」1 つに畳む**（テーマの選択と「接続先」のボタンを設定画面へ移す）| 入口が 3 つあると、どこで何が変えられるのか読めない。**リポジトリ 1 つぶんの設定は混ぜない**（右クリックのまま。CLAUDE.md §6）|
| **数値の検証は純関数（`src/lib/settingsForm.ts`）** ＋ **Rust の `normalize()` にも同じ範囲**を足す | `.tsx` に書くとテストが 1 つも当たらない（T-19）。`settings.json` は手編集を想定しているので、画面だけで締めても足りない（CLAUDE.md §5）|
| **入力が不正なときも欄を消さず、理由を欄の下に 1 行**出す | 押せない選択肢を消さないのと同じ（CLAUDE.md §6）|
| **ショートカットは対応表を純関数へ出し、既存のハンドラをそれに合わせる**（1 つのフックへ統合しない）| 統合は動いているものを全部書き直すことになる。**表を 1 か所に置けば、設定画面の一覧と実際のキーがずれない**（一覧が飾りにならない）|
| **git のパスを変えたら `detect_git` を呼び直す** | 保存しただけで「効いていない」状態を作らない。**結果（版と場所）をその場に出す** |
| **保存は欄から離れたときに行う**（skill の「追加の指示」と同じ）| 保存ボタンを増やさない。**数値が不正なときは保存しない**（前の値のまま残す）|

**作成・変更するファイル**

| 種別 | パス | 内容 |
|---|---|---|
| 新規 | `src/components/settings/SettingsDialog.tsx` | タブと枠。中身は既存の部品を並べる |
| 変更 | `src/components/settings/LlmProfiles.tsx` | ダイアログの枠を外し、**タブの中身**として使えるようにする |
| 新規 | `src/lib/settingsForm.ts` ＋ `.test.ts` | 数値と選択肢の検証・整形（**純関数**）|
| 新規 | `src/lib/shortcuts.ts` ＋ `.test.ts` | キーの対応表と `matches()`（**純関数**）|
| 新規 | `src/lib/diffSearch.ts` ＋ `.test.ts` | 差分の中の検索（**純関数**）|
| 新規 | `src/components/diff/DiffSearchBar.tsx` | 検索の欄・件数・前後移動 |
| 新規 | `src/hooks/useEscape.ts` | `Esc` の判定。**4 か所にあった写しを 1 つに** |
| 変更 | `src/components/settings/RepositorySettingsDialog.tsx` | **既定の接続先を見せる**（T-23 で覚えるようにしたが画面から読めなかった）|
| 変更 | `src/lib/llmProfile.ts` ＋ `.test.ts` | 既定の接続先の出し分け（**純関数**）|
| 変更 | `src/i18n/ja.ts` | **`.gitviewer\skills\` が `.gitviewerskills` と出ていたのを直した**（T-21 からの取りこぼし）|
| 変更 | `src/App.tsx` / `src/hooks/useCommitNavigation.ts` / `src/hooks/useFileNavigation.ts` / `src/components/diff/useWorkingTree.ts` | 対応表を使う形に寄せる ／ `Ctrl+,` を足す |
| 変更 | `src-tauri/src/store/settings.rs` | `normalize()` に数値の範囲を足す ＋ テスト |
| 変更 | `src/i18n/ja.ts` / `src/styles/app.css` | 文言・見た目 |

**実装内容**

*設定画面（`SettingsDialog.tsx`）*

- タブは **一般 / 表示 / 差分 / fetch / AI レビュー / 観点 / ショートカット / ログ**
- **一般**: git のパス（変えたら再チェックし、結果をその場に出す）／ `workspaceRoot`
- **表示**: テーマ ／ 日時形式 ／ コミットの並び順
- **差分**: レイアウト ／ 文脈行 ／ 空白無視 ／ 改行の表示 ／ 折りたたむ閾値（行・バイト）
- **fetch**: 放置警告の日数（**0 で警告しない**ことを 1 行で書く）
- **AI レビュー**: 並列度（1〜3）／ LLM へ渡す文脈行 ／ 接続先の一覧（既存の部品）
- **観点**: グローバル skill（既存の部品）。**リポジトリ内 skill はここに出さない**
- **ショートカット**: 対応表をそのまま一覧にする（**編集はしない。v1 は固定**）
- **ログ**: 置き場所 ／ 「ログフォルダを開く」／ 書けていないときは理由（T-24）

*ショートカット（`lib/shortcuts.ts`）*

- `Action` と「キーの形」の対応表を 1 つ持ち、`matches(event, action)` で判定する
- **入力欄では横取りしない**判定（`isTyping`）もここへ出す
- 足すのは **`Ctrl+,`（設定を開く）**。`Esc` は既存の扱いに合わせる
- **`Ctrl+R` は必ず `preventDefault`**（WebView のページ再読込に取られる。DESIGN.md §6.5）

*設定の検証（`lib/settingsForm.ts`）*

- 数値欄ごとに最小・最大・既定を持ち、**入力文字列 → 値 or 理由**を返す
- 範囲は Rust の `normalize()` と**同じ値**にする（片方だけ直せない形にするため、
  **範囲は 1 か所に書いてテストで突き合わせる**）

**制約**

- 表示文言は `src/i18n/ja.ts`（CLAUDE.md §6）
- **リポジトリに紐づく設定をアプリ全体の設定に混ぜない**（CLAUDE.md §6）
- **判定・検証・整形は `src/lib/*.ts` の純関数**（CLAUDE.md §8）
- **API キーの平文に触らない**（`secret.rs` と `llm/client.rs` だけ。CLAUDE.md §4）
- 新しい依存を足さない（足したくなったら実装前に利用者へ聞く）

**受け入れ条件**

- ▸コマンド: `npm run test` — 数値の検証: **空欄 / 0 / 最小 / 最大 / 最大 +1 /
  数字でない / 小数 / 前後の空白** のそれぞれで、値か理由が返ること
- ▸コマンド: `npm run test` — キーの対応表: 修飾キーの違いで**別の動作にならない**こと
  （`Ctrl+R` と `Ctrl+Shift+R`）／ 入力欄では**どれも当たらない**こと ／
  一覧に出す文字列が対応表から作られること
- ▸コマンド: `npm run test:rust` — `normalize()` が数値を締めること（**画面と同じ範囲**）／
  **範囲外の `settings.json` を読んでもファイルを捨てない**こと
- ▸コマンド: `npm run test:rust` / `npm run check:rust` / `npm run test` / `npm run typecheck`
- ▸目視: `Ctrl+,` で開き、`Esc` で閉じること ／ **DESIGN.md §6.5 の表が全部効く**こと
- ▸目視: 設定を変えると**即座に反映され**、再起動しても保持されること
- ▸目視: 不正な値を入れると**理由が出て、保存されない**こと（欄は消えない）
- ▸目視: git のパスを変えると**その場で再チェックが走り**、結果が読めること
- ▸目視: ログの場所が読め、「ログフォルダを開く」で開くこと

**⚠️ このタスクはエージェントが単独で完了を宣言してはいけない。**

**テスト**: `npm run test`, `npm run typecheck`, `npm run test:rust`, `npm run check:rust`, 目視

**非スコープ**: ショートカットの編集（v1 は固定）／ リポジトリ 1 つぶんの設定の集約
（**混ぜない**。CLAUDE.md §6）／ skill の編集 UI（v1 の範囲外）

**注記**: **`Ctrl+F`（差分内検索）の扱いは着手前に利用者へ確認する。**
DESIGN.md §6.5 の表には載っているが、これはショートカットではなく**新機能**であり、
T-25 の非スコープ「新機能の追加」と衝突する。T-23 で作った `rowIndexForLine` と
飛ぶ仕組みが使えるので、**開いているファイルの中だけ**なら収まる見込み。
