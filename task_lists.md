...
**Phase 6 は fetch まで入った。** 残りは checkout と FF マージ（T-18）で、
書き込み系の入口は `src-tauri/src/git/ops.rs` に揃っている。
...
**`.button--primary` に共通の `:hover` を効かせないこと。** 中立の `--bg-hover` が
accent を上書きし、文字が地に溶けて「押せない」ように見える（実測 1.20 / 1.30）。
