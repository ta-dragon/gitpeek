...
| いまここ | **T-24 ログファイルとエラー処理の仕上げ。実装とテストは通った。目視待ち**（下の表）|
| 未解決の判断事項 | なし。実装レベルの選択は 2026-09-06 に利用者が決めた（T-24 の節）|

**Phase 8 が終わり、残るは Phase 9（T-24 → T-25 → T-26）だけ。**
**T-25 と T-26 はまだ骨子**なので、着手時に詳細化そのものを最初の作業として行うこと（規約 5）。

### T-24 の目視の進み具合（2026-09-06 時点）

| # | 見るもの | 状態 |
|---|---|---|
| 1 | 起動すると `%APPDATA%\com.tatsu.givsoner\logs\givsoner-YYYY-MM-DD.log` ができ、git の実行が 1 行ずつ積まれる | 未 |
| 2 | 認証情報付き URL のリモートで fetch / clone し、**画面にもログにも平文が出ない** | 未 |
| 3 | 日付を古くしたログを置いて起動すると消える ／ **同じフォルダの他のファイルは残る** | 未 |
| 4 | フロントで例外を起こすと、白い画面にならず**ログにも 1 行残る** ／ ログの場所が読める ／「ログフォルダを開く」で開く | 未 |
| 5 | Rust 側で panic を起こすと**ダイアログが出て**、理由とログの場所が読める | 未 |
| 6 | ログに**本文が出ていない**こと（プロンプト・差分・レビュー本文・skill 本文）| 未 |

**5 を試すには panic を仕込む必要がある。** 一時的にコマンドの中へ `panic!("試験")` を
置いて確かめ、**確かめたら消すこと**（コミットに混ぜない）。
...
| 8 | 9 | **T-24** → T-25 → T-26 | いまここ。**T-24 は実装済み・目視待ち**。T-25・T-26 は着手前に詳細化（規約 5）|
...
**目的**: 後から追える記録を残し、落ちたときに何が起きたのかを画面から辿れるようにする。

**参照**: DESIGN.md §13.3, §13.4, §13.5 / CLAUDE.md §4, §5, §6

**依存**: T-18, T-19, T-23

**着手時に確認した現況（規約 4。2026-09-06）**

- **`StorePaths::logs_dir()` は既にあり、`ensure()` が `logs\` まで作る。**
  無いのは**書くコードだけ**。置き場所は決まっている（CLAUDE.md §5）
- **git 実行の記録は `commandlog.rs` に集約済み。** `exec.rs` は `LogSink` 越しにしか
  記録しないので、**ログファイルの口もここに足すのが素直**（別経路にすると
  マスキングが 2 か所になる）
- **マスキングはすでに広く通っている。** git の stderr・進捗・失敗メッセージ（`git/ops.rs`）、
  レビューの `baseUrl`（`store/reviews.rs`）、LLM の応答（`llm/client.rs` の `sanitize`）。
  **通っていないのは「まだ無いログファイル」だけ**なので、T-24 の点検は
  「新しい口を通す」ことと「取りこぼしを洗い出す」ことの 2 つ
- **白い画面の受け皿は済んでいる**（`index.html` ＋ `ErrorBoundary`。DESIGN.md §13.5）。
  T-24 で足すのは **Rust 側の panic** と、**フロントの例外をログへ残すこと**
- **`tauri-plugin-opener` は既に依存にある**（capability は `opener:allow-open-url` だけ許可）。
  ログフォルダを開くなら **capability を 1 行足すだけ**で済み、依存は増えない

**実装レベルの選択（2026-09-06 に利用者が決めた）**

**着手前に 3 つ聞き、うち 2 つは提案と違う答えだった。** 決まった内容で実装してある。

| 決めたこと | 誰が / なぜ |
|---|---|
| **`tauri-plugin-log` を足す**（Cargo に `tauri-plugin-log` と `log` の 2 つ。npm は増やさない）| **利用者の判断。** 自前（依存ゼロ）と並べて聞いた結果。フロントの例外は自前のコマンド 1 つで Rust へ渡すので、npm 側の依存は増えていない |
| **git の stderr は全文残す** | **利用者の判断。** 認証まわりは再現が難しく、1 行目では足りない。画面のリングバッファ（500 件）は流れていく |
| **「ログフォルダを開く」を付ける** | **利用者の判断。** ただし **capability は広げずに済んだ** — 開く操作を Rust 側のコマンドに閉じ、パスをフロントから渡さない形にした（`opener:allow-open-path` を足すと画面側から任意のパスを開けてしまう）|
| **書き出しの口は `logging::format_line` の 1 つだけ。そこで `redact` を通す** | プラグインの `format` にこれを差し込んであるので、`log` へ流れたものは全部通る。2 か所から書くと片方でマスクを忘れる（CLAUDE.md §4）|
| **書くのは「何が起きたか」だけ。本文は書かない** | プロンプト・差分・レビュー本文・skill 本文を書くと、**%APPDATA% に他人のソースと秘密が残る**。ログの目的は後から追えることであって、再現できることではない。LLM はメタ情報（接続先・モデル・所要時間・結果の種別）だけ |
| **1 行 1 イベントの素のテキスト**（`時刻 レベル 種別 本文`）| 読むのは作者 1 人で、要件は grep できること。JSON Lines は目で読めない |
| **ログに書けなくてもアプリは止めない。ただし 1 度だけ画面に出す** | ログのためにアプリが動かないのは本末転倒。黙って落とすと「書いているつもり」になる |
| **panic は `std::panic::set_hook` で捕まえ、ログへ書いてからフロントへ知らせる** | Tauri コマンドの中で落ちると IPC のエラーとしてしか見えない。**ダイアログにログの場所を出す**（DESIGN.md §13.3）|
| **フロントの例外もログへ 1 行送る**（`ErrorBoundary` から） | 白い画面の受け皿は画面には出るが、閉じると何も残らない。**Rust 側と同じファイルに並ぶ**ほうが辿りやすい |
| **「ログフォルダを開く」ボタンを付ける**（capability に `opener:allow-open-path` を足す）| パスを文字で出すだけでは Windows で辿りにくい。**依存は増えない**（プラグインは導入済み）|
| **保持は 7 日固定。設定に出さない** | DESIGN.md §13.3 の決めをそのまま実装する。場所を設定画面に出すのは T-25 |
| **古いものを消すのは自前の掃除だけ**（プラグインの回転には消させない）| 消してよいのは `givsoner-YYYY-MM-DD` で始まり `.log` で終わるものだけ。フォルダに置かれた他のファイルに触らせない |

**作成・変更するファイル**

| 種別 | パス | 内容 |
|---|---|---|
| 新規 | `src-tauri/src/logging.rs` | 書き出しの口 ／ プラグインの組み立て ／ 7 日の掃除 ／ panic hook。**マスキングはここで通す** |
| 変更 | `src-tauri/src/git/exec.rs` | **stderr を組み立てるところで `redact` を通す**（点検で見つかった口）|
| 変更 | `src-tauri/src/commandlog.rs` | 積むときにログへも 1 行書く ＋ `log_line`（純関数）|
| 変更 | `src-tauri/src/llm/client.rs` | LLM のメタ情報を 1 行（**本文は書かない**）|
| 変更 | `src-tauri/src/lib.rs` | 起動時の初期化 ／ git 検出と設定の結果 ／ `log_status` / `open_log_folder` / `log_frontend_error` |
| ~~変更~~ | ~~`src-tauri/capabilities/default.json`~~ | **触らずに済んだ。** 開く操作を Rust 側に閉じたので `opener:allow-open-path` は要らない |
| 新規 | `src-tauri/tests/logging.rs` | **`log` の口からファイルまで繋がっていること** ／ 同時に書いても行が壊れないこと |
| 変更 | `src/components/common/ErrorBoundary.tsx` | 例外をログへ送り、ログの場所と「開く」を出す |
| 変更 | `src/App.tsx` | panic の知らせ ／ 書けていないときのバナー |
| 変更 | `src/i18n/ja.ts` / `src/lib/ipc.ts` / `src/styles/app.css` | 文言・配線・見た目 |
| 新規 | `src/lib/crashNotice.ts` ＋ `.test.ts` | 出す文言の組み立て（**純関数**。`.tsx` に判定を書かない。CLAUDE.md §8）|

**実装内容**

*書き込み（`logging.rs`）*

- ファイルは `logs\givsoner-YYYY-MM-DD.log`。**追記で開いて閉じる**（開きっぱなしにしない —
  別のプロセスやエディタから読めるように）。日付が変わればファイル名が変わるので、
  切り替えの判定を別に持たない
- **全行が `redact` を通る。** 呼び出し側は何もしなくてよい
- 掃除は**起動時に 1 回**。`givsoner-YYYY-MM-DD.log` の形に合うものだけを見て、
  7 日より古いものを消す。**形が違うファイルには触らない**（フォルダに置かれた
  他のものを消さない）。消せなくても失敗にしない
- **書けなくても `Err` を上へ返さない。** 1 度だけ画面へ知らせて、以後は黙る

*何を書くか*

| 種別 | 中身 |
|---|---|
| `git` | `commandlog.rs` が積む 1 件（コマンド・exit code・所要時間・stderr の 1 行目）|
| `app` | 起動 ／ git の検出結果 ／ リポジトリの読み込み失敗 ／ 設定の読み書きの失敗 |
| `llm` | 接続先・モデル・所要時間・結果の種別（**本文・プロンプト・差分は書かない**）|
| `panic` | メッセージと発生位置（**ペイロードも `redact` を通す**）|
| `ui` | フロントの例外（`ErrorBoundary` から届いたもの）|

*panic（`lib.rs`）*

- `std::panic::set_hook` で捕まえ、ログへ書いてから `app-panic` イベントを送る
- フロントは**ログの場所を添えたダイアログ**を出す（DESIGN.md §13.3）。
  **「フォルダを開く」を置く**（capability に `opener:allow-open-path`）
- **hook は既定の挙動も残す**（標準エラーへの出力を消さない。`tauri dev` で見えなくなる）

*点検（マスキングの適用範囲）*

- 画面へ出る経路とログへ出る経路を**洗い出して表にする**（DESIGN.md §13.4 へ残す）。
  いま通っているもの（git の stderr・進捗・失敗メッセージ／レビューの `baseUrl`／
  LLM の応答）と、**通っていないもの**を分けて書く
- **`settings.json` / `state.json` / `reviews\` に平文が無いこと**は T-20・T-23 で
  テスト済み。ここでは**ログファイル**を同じやり方（ファイルを読んで確かめる）で見る

**制約**

- **全出力が `redact.rs` を通ること**（CLAUDE.md §4）。%APPDATA% に平文トークンを残さない
- ログは `%APPDATA%\com.tatsu.givsoner\logs\`。**OneDrive 配下に置かない**（CLAUDE.md §5）
- **新しい依存を足さない**（足したくなったら実装前に利用者へ聞く）
- 判定・整形は `src/lib/*.ts` の純関数。`.tsx` に書かない（CLAUDE.md §8）
- 表示文言は `src/i18n/ja.ts`。**ただし `index.html` の受け皿はそのまま**（DESIGN.md §13.5）

**受け入れ条件**

- ▸コマンド: `npm run test:rust` — 日付ごとにファイルが分かれること ／
  **7 日より古いものだけが消え、ちょうど 7 日は残る**こと ／
  **`givsoner-YYYY-MM-DD.log` の形でないファイルに触らない**こと ／
  **書けないディレクトリでも panic せず、アプリが動き続ける**こと
- ▸コマンド: `npm run test:rust` — **ログファイルに平文の資格情報が 1 文字も無い**こと
  （`tests/logging.rs`。書き出しの口を通してファイルへ書き、**読み直して確かめる**。
  **伏せ字になった形まで見る** — 「含まれない」だけだと、そもそも書けていないときも通る）
- ▸コマンド: `npm run test:rust` — **複数スレッドから同時に書いても行が壊れない**こと
  （行数が一致し、途中で混ざった行が無い）
- ▸コマンド: `npm run test:rust` — **git の実行がログの口までつながっている**こと
  （`CommandLog::record` → `log` → ファイル）／ panic の 1 行が組み立てられること
- ▸コマンド: `npm run test:rust` — **git が吐いた stderr に平文が残らない**こと
  （`tests/fetch.rs`。**git が伏せてくれない形**で試す — `fetch <資格情報付き URL>` では
  git 自身が資格情報を落とすので、**マスクを外しても緑のまま通ってしまう**）
- ▸コマンド: `npm run test` — 出す文言の純関数（ログの場所がある ／ 無い ／
  **例外のメッセージが空**）
- ▸コマンド: `npm run test:rust` / `npm run check:rust` / `npm run test` / `npm run typecheck`
- ▸目視: 認証情報付き URL のリモートで fetch / clone し、**画面にもログファイルにも
  平文が出ない**こと
- ▸目視: 意図的に panic を起こすと**ダイアログが出て、ログの場所が読める**こと ／
  「フォルダを開く」でエクスプローラーが開くこと
- ▸目視: 日付を古くしたログを置いて起動すると**消えている**こと ／
  **同じフォルダに置いた別のファイルは残っている**こと
- ▸目視: フロントで例外を起こすと、白い画面にならず**ログにも 1 行残る**こと

**⚠️ このタスクはエージェントが単独で完了を宣言してはいけない。**

**テスト**: `npm run test:rust`, `npm run check:rust`, `npm run test`, `npm run typecheck`, 目視

**非スコープ**: 設定画面（T-25）／ ログの詳細度の切り替え ／
テレメトリ・クラッシュレポートの外部送信（**恒久的に非対象**。外部通信は git と LLM だけ）

**注記**: 3 日基準に収まる見込み。**点検（マスキングの適用範囲）が本体**で、
ログを書くこと自体は道具立てにすぎない。**T-23 までで平文が漏れうる口は塞いである**ので、
ここで増える口（ログファイル）を最初から塞いだ状態で作ることが要点。
