...
5. **骨子のままのタスクは、着手前に詳細化する。** ⑥実装内容と⑧受け入れ条件が埋まって
   ...
   **いま骨子のタスクは無い**。未着手は **T-36**（コード内容で探す）だけで、2026-09-14 に詳細化済み。
...
1. 下の「▸コマンド」を 4 つ流してベースラインを取る（直近は **Rust 510 / vitest 456** で全部緑）
2. **T-36 を進める**（「v1.1 — コミットを探す」の節）。T-35 は 2026-09-17 に目視が通って完了した。
   **2026-09-14 に利用者がコミット検索を次の作業に決めた**。目安の時間・中止の扱いは
...
| 項目 | 内容 |
|---|---|
| 直前に完了 | **T-35**（ことばでコミットを探す。2026-09-17）/ T-26（v1 の DoD。2026-09-12）|
| いまここ | **T-36 着手**（コード内容で探す ＋ 目安の時間と中止）|
| 未解決の判断事項 | なし（T-36 の決めごとは 2026-09-14 に確定）。ただし**文言はまだ途中で、利用者はまだ満足していない**（下記）|
...
**2026-09-14 に利用者が着手を決めた。** DESIGN.md §15.3 の「コミット検索」を発番したもの。
**T-35（メッセージ / 作者）は 2026-09-17 に完了**（末尾の「完了済み」）。形と実測は DESIGN.md §6.6。

## - [ ] T-36 [v1.1] コード内容で探す（`log -S`）＋ 目安の時間と中止

**目的**: `code:` を実装する。**重い**（onyx で 17 秒）ので、**目安の時間を出し、中止できる**ようにする。

**参照**: DESIGN.md §6.6（打ち方・呼び分け・実測）/ §3.1（exec の入口）/ §8.3（中止の作り）/ CLAUDE.md §2, §6
...
1. **中止できるようにする。** `exec::run_streaming` に**中止の旗（既存の `Cancel`）を足す** —
   ...
   （`git/search.rs` のほうが旗を渡す）
2. **完了までの目安の時間を出す**（利用者の指定）。材料は**コミット件数 ÷ レート**で、
   - 初回のレートは **1,200 コミット/秒**（onyx の実測 20,285 / 17.1 秒 ≒ 1,190。DESIGN.md §6.6。そこから、**遅い側**に寄せる。
...
4. `code:` と `message:` が一緒なら**必ず同じ 1 回の git 実行にまとめる**（17.1 秒 → 5.9 秒。DESIGN.md §6.6 の実測）
...
| T-35 | v1.1 | ことばでコミットを探す（メッセージ / 作者。**`--author` を 2 つ並べると `--all-match` でも OR** — DESIGN.md §17.1。判定はフックから `planSearch` / `searchView` へ出した。DESIGN.md §6.6）| （このコミット）|
