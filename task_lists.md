...
**目的**: 複数リポジトリを登録して素早く切り替えられるようにし、アプリ全体のレイアウト骨格を確定する。

**参照**: DESIGN.md §6.1, §6.2, §13.1, §13.2 / CLAUDE.md §6
...
| 種別 | パス | 内容 |
|---|---|---|
| 新規 | `src/components/sidebar/Sidebar.tsx` | サイドバー枠（上段=リポジトリ、下段=ref ツリーの場所） |
| 新規 | `src/components/sidebar/RepositoryList.tsx` | 一覧・追加・削除・並べ替え |
| 新規 | `src/components/common/CommandPalette.tsx` | `Ctrl+P` 絞り込みジャンプ |
| 新規 | `src/components/common/SplitPane.tsx` | 境界ドラッグで比率可変 |
| 新規 | `src/components/setup/EmptyState.tsx` | 3 ボタンの空状態画面 |
| 新規 | `src/store/repositories.ts` | リポジトリ状態 |
| 変更 | `src/App.tsx` | レイアウト骨格 |
| 変更 | `src/styles/app.css`, `src/i18n/ja.ts` | |
| 変更 | `package.json`, `src-tauri/Cargo.toml`, `src-tauri/capabilities/default.json` | dialog プラグイン追加 |

**実装内容**

- **レイアウト骨格**: 左サイドバー（幅可変・`state.json` に永続化）＋ 中央 ＋ 下部コマンドログ。
  中央の上下分割は T-07 で入れるので、ここでは 1 枚の領域でよい。
- **フォルダ選択**に `@tauri-apps/plugin-dialog` を追加する。Rust 側 `tauri-plugin-dialog`、
  capabilities に `dialog:allow-open` を追加。
- **一覧の各行**: リポジトリ名 / パス（中間を省略表示）。パスが消えている場合はグレーアウトし、
  「再指定」「登録解除」を出す。ahead/behind バッジと dirty マークの**場所だけ確保**しておく
  （中身は T-17 / T-16）。
- **空状態**: 「リポジトリを追加」「フォルダをスキャン」「URL から clone」の 3 ボタン。
  clone は T-19 まで無効化し、ツールチップで「未実装」と示す。
- **`Ctrl+P`**: インクリメンタル検索（名前とパスの部分一致）、`↑↓` 選択、`Enter` 決定、`Esc` 閉じる。
- **状態復元**: 起動時に `state.json` の `lastRepositoryId` を開く。パスが消えていたら空状態へ。
- **並べ替え**: 手動 D&D ＋「最終アクセス順」トグル。順序は `settings.json` の `order` に保存。

**制約**

- **全表示文言は `src/i18n/ja.ts` に集約**し、コンポーネントに直書きしない（CLAUDE.md §6）
- 色は `src/styles/theme.css` のトークン経由で参照する。生の色値を書かない（CLAUDE.md §6）

**受け入れ条件**

- ▸コマンド: `npm run typecheck`
- ▸コマンド: `grep -rnE '#[0-9a-fA-F]{3,6}\b' src/components` が 0 件（色の直書きが無い）
- ▸目視: リポジトリを 3 個以上登録し、`Ctrl+P` で切り替えられる
- ▸目視: 登録済みリポジトリのフォルダをリネームすると一覧でグレーアウトし、再指定・登録解除ができる
- ▸目視: 再起動すると前回のリポジトリが選択されている
- ▸目視: リポジトリ 0 件のとき空状態画面が出る
- ▸目視: サイドバー幅を変えて再起動すると保持されている

**非スコープ**

コミットグラフとリスト（T-07）／ブランチツリーの中身（T-10）／clone の実処理（T-19）／
ahead/behind とdirty の実データ（T-17, T-16）／フォルダグループ分け（v1.1）
...
for-each-ref --format=%(objectname)%x1f%(refname)%x1f%(objecttype)%x1f%(*objectname)%x1f%(upstream) refs/heads refs/remotes refs/tags
symbolic-ref -q --short refs/remotes/origin/HEAD
```

- レコード区切りは `-z`（NUL）、フィールド区切りは `%x1f`（Unit Separator）。
  **`-z` が `--format` 出力にも効くことを、改行を含むコミットメッセージのテストリポジトリで確認すること。**
  効かない場合は各レコードの先頭に `%x1e`（Record Separator）を置いて分割する方式に切り替える。
