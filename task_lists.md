...
- **窓が行 0 から始まらなくなる点に注意。** 現在は先頭から描いているので、素通りする
  レーンの縦線は「上の行のノードから下の行の親まで」の辺がすべて覆っており、
  `row.passing` を描く必要がない（描くと切れ端が残るのでやめた）。
  仮想スクロールで窓が途中から始まると、**窓の上から入ってくる辺が描かれなくなる**。
  窓を跨ぐ辺を拾って描くこと。`passing` を縦線として描き直す場合は、
  **辺が覆っている行には引かない**（重ねると切れ端が戻る）。
