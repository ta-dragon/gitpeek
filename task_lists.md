...
| 項目 | 内容 |
|---|---|
| 直前に完了 | T-10 ブランチ / タグツリー UI（**Phase 3 完了**） |
| 次にやる | **T-11 コミット詳細と変更ファイル一覧 — 実装は終わり、目視確認待ち** |
| 未解決の判断事項 | なし |

**T-11 は ▸コマンド の受け入れ条件を満たしたところで止めてある。** ▸目視 が 4 項目
...
- **`Enter`（差分ペインへフォーカス）と `Alt+↑` `Alt+↓`（前 / 次のファイルへ）は
  T-07 で保留した**（差分ペインが無かったため）。DESIGN.md §6.5 の表がショートカットの正。
...
| 種別 | パス | 内容 |
|---|---|---|
| 新規 | `src-tauri/src/git/diff.rs` | `show -s` と `--raw --numstat` の実行とパース |
| 変更 | `src-tauri/src/git/mod.rs` | `pub mod diff;` |
| 変更 | `src-tauri/src/lib.rs` | `load_commit_detail` / `load_changed_files` の登録 |
| 新規 | `src-tauri/tests/diff.rs` | 生成リポジトリでの結合テスト |
| 変更 | `scripts/make-test-repos.sh` | リネーム / バイナリ / 日本語名を含むリポジトリ |
| 新規 | `src/lib/fileTree.ts` + `.test.ts` | パス一覧 → ディレクトリツリー（純関数） |
| 新規 | `src/components/diff/DiffPane.tsx` | 3 段の器。取得とファイル選択 |
| 新規 | `src/components/diff/CommitDetail.tsx` | 1 段目 |
| 新規 | `src/components/diff/FileList.tsx` | 2 段目 |
| 変更 | `src/lib/ipc.ts` / `src/i18n/ja.ts` / `src/App.tsx` / `src/styles/app.css` | 配線 |

**実装内容**

*Rust 側*

- **コミット本文**: `show -s --format=<fmt> <sha>`。`%H %h %P %an %ae %at %cn %ce %ct %s %b` を
  `%x1f` 区切りで取り、`%b`（本文）を最後に置く。
- **変更ファイル一覧**: **`--raw` と `--numstat` を 1 回の実行で両方出す。**
  `diff --raw --numstat -z -M <parent> <sha>`。raw から状態（A/M/D/R）とファイルモード、
  numstat から増減行数とバイナリ判定が取れる。2 回呼ぶ必要はない（実測で確認）。
- **ルートコミット（親なし）** は `diff` では出せないので
  `diff-tree --raw --numstat -z -M --root --no-commit-id -r <sha>` を使う。
  API では親を `Option<String>` にし、`None` がルートを意味する。
- **マージは呼び出し側が親を明示する。** `--cc` は使わない（DESIGN.md §7.4）。
...
**raw と numstat は 1 本の NUL 列に連結されて出る。** 先頭が `:` の間が raw で、
そうでないレコードが来たら numstat 区間。リネーム / コピー（`R` / `C`）のレコードだけ
パスを 2 つ食うので、そこを取り違えると以降が全部ずれる。

*フロント側*

- 差分ペインは 3 段（コミット詳細 → ファイル一覧 → 選択ファイルの差分）。
  **3 段目は T-13 まで空**の器を置く。
- ファイル一覧は**フラット（既定）／ツリー**の切替。フラットは共通ディレクトリを淡色にして
  ファイル名を強調する（DESIGN.md §7.3）。ファイルごとに `+N` `-M` と増減バー、
  ヘッダに合計とファイル数。
- **マージコミットはヘッダに親のドロップダウン**。既定は第 1 親。
  **親の subject は再取得しない** — 全コミットが手元にあるので `commits` から引く。
- `Alt+↑` / `Alt+↓` で前 / 次のファイル、`Enter` で差分ペインへフォーカス。
- 選択ファイルは `state.json` の `selectedFile` に持つ（リポジトリごと）。
...
- **スクロールするペインの `flex-basis` は 0**（CLAUDE.md §6。T-10 で潰れた）

**受け入れ条件**

- ▸コマンド: `npm run test:rust` — パーサの単体テスト（追加 / 変更 / 削除 / リネーム /
  バイナリ / 日本語ファイル名 / サブディレクトリ / 空の出力）と、
  生成リポジトリでの結合テスト（ルートコミット / マージコミットの親切替）
- ▸コマンド: `npm run test` — `fileTree` の階層化
- ▸コマンド: `npm run typecheck` / `npm run check:rust`
- ▸目視: コミットを選ぶと詳細と変更ファイル一覧が出る
- ▸目視: マージコミットで親を切り替えると一覧が変わる
- ▸目視: 日本語ファイル名が化けない、リネームが `old → new` で出る
- ▸目視: `Alt+↑` `Alt+↓` でファイルを移動できる

**T-11 で決めたこと（DESIGN.md §7.3, §7.4 に転記済み）**

- **一覧は 1 回の git 実行で作る。** `--raw` と `--numstat` は 1 本の NUL 列に続けて出る
  （実測）。2 回呼ぶ必要はない。
- **ルートコミットは `diff-tree --root`。** 空ツリーの SHA を定数で埋め込まない
  （SHA-256 リポジトリで値が変わる）。API は親を `Option<String>` にした。
- **バイナリの増減は `null`。** 0 行の変更と区別する。合計にも足さず別に数える。
- **マージの親ドロップダウンは見出し行に置く。** facts の欄だと本文が長いときに隠れ、
  どの親との差分か分からないまま一覧を読むことになる（目視で確認）。
- **選んだ親は永続化しない。** コミットが変われば意味を失う値なので、第 1 親へ戻す。
- **ディレクトリツリーは子が 1 つの連なりを 1 行にまとめる**（`src/components/diff`）。
  開閉は持たない（1 コミットの変更は高々数十件）。
- **フラット表示で削るのはディレクトリ側だけ。** ファイル名は必ず残す。
- **ファイル移動と `Enter` は `DiffPane` が処理する。** ファイル一覧を持っているのが
  そこなので、`useCommitNavigation` へ状態を渡すより素直。

**T-11 で見つけて直した不具合**

**フラット表示でディレクトリがファイル名の後ろに出た（目視 1 巡目）。** 末尾を残すために
`direction: rtl` を使ったが、インライン要素の並びまで反転する。ディレクトリ側だけを
`text-overflow: ellipsis` で削る形に変えた。

**変更ファイル一覧が 1〜2 行しか見えなかった（目視 1 巡目）。** `.flist__body` に
`min-height` を書いても、親の `min-height: 0` が min-content の制約を切るので効かない。
下限は `.flist` 側に置いた。

**非スコープ**

差分本体の描画（T-13）／シンタックスハイライト（T-14）／作業ツリー（T-16）／
任意 2 コミット間差分（T-15）
...
- **3 段目の器はもう置いてある。** `DiffPane.tsx` の `.dpane__body` が
  プレースホルダになっているので、そこを差し替える。
- **選択中のファイルは `state.json` の `selectedFile` が正**（`uiState` ストア）。
  差分側で別に `useState` を持たないこと。
- **比較する親は `DiffPane` の `parentIndex`**（永続化しない）。差分本体の取得にも
