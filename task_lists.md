...
| 項目 | 内容 |
|---|---|
| 直前に完了 | T-28 `Givsoner` → `GitPeek` の改名（目視 2026-09-06 通過）|
| いまここ | **T-26 配布ビルドと DoD 検証。これで v1 が終わる**。詳細化済み |
| 未解決の判断事項 | なし |

**残るは T-26 だけ。** ただし **DoD は実運用 1 週間**なので、作業時間ではなく
カレンダー時間で終わる（規約の 3 日基準の例外）。**zip を作るところまでは
すぐ終わり、そのあと 1 週間使う。**
