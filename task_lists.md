...
残っているのは T-21 節の ▸目視 4 件で、**リポジトリ内に `.gitviewer/skills/*.md` を
置いたリポジトリ**が要る（未信頼と読めること、確認画面で本文が全部読めること、
ファイルを 1 つ足すと再確認を求められること、skill が無くても何が効くか読めること）。
...
- ▸コマンド: `npm run test:rust` — 信頼したあとで **①内容を書き換える ②ファイルを増やす**
  のそれぞれで**再確認が要求される**こと。**③ファイルを消したときは要求されない**こと
...
- ▸コマンド: `npm run test:rust` — 同名のとき、**信頼済みならリポジトリ内 / 未信頼なら
  グローバル**が使われること
...
- ▸目視: リポジトリ内 skill を置いたリポジトリを開くと、**一覧に出るが未信頼**と読めること
- ▸目視: 信頼の確認画面で**本文が全部読める**こと（読まずに信頼させない）
- ▸目視: 信頼したあとでファイルを 1 つ足すと、**次に開いたときに再確認を求められる**こと
...
**既定は「配らない」**にしてあり、`load()` が信頼を確かめたときにだけ渡す
...
**再確認の間は、そのリポジトリの skill を全部止める。** 変わったファイルだけを止める形も
考えたが、どのファイルのどの変化が効くのかは中身を読まないと分からないので選り分けない。

**読めないファイルもハッシュに数える。** 数えないと、**読めないファイルを足せば
再確認を素通りできる**。`adding_an_unreadable_file_after_trusting_still_asks_again` が固定。

**skill が無くても「信頼の取り消し」は押せるようにした。** ファイルを消したあとも記録は
残るので、押せないと信頼が残っていることに気付けない（あとで置き直したファイルが即座に効く）。

**画面は設定モーダルのタブ 1 つ**（`RepoSkills.tsx`）。本文の `SkillPicker.tsx` は
実行前パネル用なので T-23 へ回した。
