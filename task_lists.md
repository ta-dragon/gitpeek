...
**ダークの差分色が見分けられなかった（目視 1 巡目）。** ペインの地との輝度比が 1.2 程度
しかなく、**特に緑が青みの強い地に埋もれていた**。地との比を 1.43〜1.59、語の強調は
行の地との比を 1.9 以上に取り直した（実測値は DESIGN.md §7.2 の表）。ライトは変更していない。

**改行の可視化記号が読み取れなかった（目視 1 巡目）。** `␍␊` のような制御文字の記号は、
並べてもどちらがどちらか分からない。**サクラエディタに合わせて CRLF は `→↓`、LF は `↓`、
CR は `→`** にした。末尾に改行が無いことは記号をやめて「改行なし」と書く。
