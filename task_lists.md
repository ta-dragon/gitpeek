...
**目視で見つかった不具合（2026-09-03、2 件とも修正済み）**

- **ホバーの背景がグラフを覆っていた。** 行の背景は不透明で、グラフ（背面の 1 枚 SVG）を
  塗り潰していた。選択の背景は半透明だったので気付かなかった。**`.graph__clip` を行より
  前面（`z-index: 1`）に出した。** `pointer-events: none` なのでクリックとホバーは
  下の行にそのまま届く。
- **横スクロールバーが 2 本出て、横スクロールすると見出しと行がずれていた。**
  列見出しがスクロールコンテナの外にいたため、(1) 横スクロールしても見出しが動かない、
  (2) 見出しの最小幅が外へ伝わって親の `.split__pane`（`overflow: auto`）に 2 本目の
  スクロールバーが生える、の 2 つが同時に起きていた。**見出しを行と同じスクロール
  コンテナへ入れ、縦だけ `position: sticky` で貼り付けた。** 高さは `--head-height`
  （26px）で固定し、仮想スクロールに `scrollMargin` として同じ値を渡す。
  併せて `.commits` を `overflow: hidden` にして最小幅が外へ漏れないようにし、
  道具立ては折り返す（`flex-wrap: wrap`）ようにした。窓を狭めても並び順や件数が消えない。
- ついでに行の幅を `max-content` にした。`width: 100%` だと横スクロールした先で
  ホバーと選択の帯が途切れる。
- 上端へ寄せるスクロールは `scrollToIndex` をやめて自前で計算している。
  `scrollToIndex` は貼り付いた見出しを知らないので、`k` や `Home` で上へ戻ったときに
  選択行が見出しの下へ潜る。
