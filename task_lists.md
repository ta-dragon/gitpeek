...
| いまここ | **T-25 設定画面とショートカット。詳細化は済み。実装レベルの選択を確認中**|
| 未解決の判断事項 | **`Ctrl+F`（差分内検索）を T-25 でやるか v1.1 へ送るか** ／ ヘッダの入口を設定画面へ畳むか |
...
| 8 | 9 | T-24 → **T-25** → T-26 | いまここ。T-24 は済。**T-25・T-26 は着手前に詳細化**（規約 5）|
