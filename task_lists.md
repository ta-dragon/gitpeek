...
**T-24 は詳細化済み**（2026-09-06）。節の「この節で決めた実装レベルの選択」は
**着手前に利用者の確認が要る**（T-23 と同じ形）。**T-25 と T-26 はまだ骨子**なので、
着手時に詳細化そのものを最初の作業として行うこと（規約 5）。
...
| 8 | 9 | **T-24** → T-25 → T-26 | いまここ。**T-24 は詳細化済み**（実装レベルの選択は要確認）。T-25・T-26 は着手前に詳細化（規約 5）|
...
| 決めたこと | 理由 |
|---|---|
| **ログは自前で書く。新しい依存を足さない** | 要るのは「1 行足す ／ 日付でファイルを分ける ／ 7 日より古いものを消す」の 3 つだけ。`tauri-plugin-log` は依存と引き換えに標準出力や webview への配信も付いてくるが、使うのは 1 用途（T-22・T-23 と同じ判断）|
| **書き込み口は `logging.rs` の 1 つだけ。そこで `redact` を通す** | 2 か所から書くと片方でマスクを忘れる。`commandlog.rs` が `record()` の中でだけマスクしているのと同じ構造にする（CLAUDE.md §4）|
...
| 種別 | パス | 内容 |
|---|---|---|
| 新規 | `src-tauri/src/logging.rs` | 書き込み口 ／ 日付でのファイル分け ／ 7 日の掃除。**マスキングはここで通す** |
| 変更 | `src-tauri/src/commandlog.rs` | 積むときにログへも 1 行書く（`LogSink` の実装側） |
| 変更 | `src-tauri/src/lib.rs` | 起動時の初期化 ／ panic hook ／ `open_log_folder` / `log_frontend_error` |
| 変更 | `src-tauri/capabilities/default.json` | `opener:allow-open-path` |
...
- ▸コマンド: `npm run test:rust` — **ログファイルに平文の資格情報が 1 文字も無い**こと
  （認証情報付き URL を含む行を書いてから**ファイルを読んで確かめる**）
- ▸コマンド: `npm run test:rust` — **複数スレッドから同時に書いても行が壊れない**こと
  （行数が一致し、途中で混ざった行が無い）
- ▸コマンド: `npm run test:rust` — git を 1 回実行するとログに 1 行増えること ／
  **panic hook がログへ書く**こと（hook を直接呼ぶ）
