...
| 項目 | 内容 |
|---|---|
| 直前に完了 | T-13 差分の取得・パースと side-by-side 描画 |
| 次にやる | **T-14 Shiki ハイライトと大差分の折りたたみ — 実装は終わり、目視確認待ち** |
| 未解決の判断事項 | なし |

**T-14 は ▸コマンド の受け入れ条件を満たしたところで止めてある。** ▸目視 が 4 項目
...
| 3 | 3 | T-09 → T-10 | **済** |
| 4 | 4 | T-11 → T-12 → T-13 → **T-14** | T-11 / T-12 / T-13 は**済** |
| 5 | 5 | T-15 / T-16 | 順不同 |
...
**参照**: DESIGN.md §7.2 / CLAUDE.md §1, §6, §8

**依存**: T-13

**T-13 からの申し送り（着手時に確認すること）**

- **仮想化はまだ入っていない。** `SideBySide` / `Unified` は hunk の行を全部 DOM にする
- `pairHunk` / `wordSegments` は hunk ごとに `useMemo` で 1 度だけ回している。行ごとに呼ばないこと
- ハイライトを挿す先は `WordDiff.tsx` の 1 箇所（本文を描いているのはここだけ）
- 表示の切替は `settings.json` の `ui`（`diffLayout` / `contextLines` / `ignoreWhitespace` /
  `showLineEndings`）。折りたたみの閾値もここへ足す
- スクロールコンテナは `DiffPane.tsx` の `.dpane__body`

**作成・変更するファイル**

| 種別 | パス | 内容 |
|---|---|---|
| 新規 | `src/lib/highlight.ts` + `.test.ts` | Shiki の生成、拡張子 → 文法の対応、トークンと語単位差分の合成 |
| 新規 | `src/lib/diffRows.ts` + `.test.ts` | hunk 群 → 平坦な行リスト（仮想スクロールの入力）と規模の計測 |
| 新規 | `src/components/diff/DiffBody.tsx` | 仮想スクロールの器（side-by-side / unified 共通） |
| 変更 | `src/components/diff/SideBySide.tsx` / `Unified.tsx` | 1 行を描くだけにする |
| 変更 | `src/components/diff/WordDiff.tsx` | トークンの色を載せる |
| 変更 | `src/components/diff/DiffPane.tsx` | 折りたたみの判定と展開状態 |
| 変更 | `src-tauri/src/git/diff.rs` / `lib.rs` | バイナリのサイズ変化（`cat-file -s`） |
| 変更 | `src/lib/ipc.ts` / `src/i18n/ja.ts` / `src/styles/app.css` / `theme.css` / `package.json` | 配線 |

**実装内容**

*ハイライト*

- `shiki/core` を使い、**文法は言語ごとに動的 import する**（`shiki/langs/*.mjs`）。
  `shiki` のフルバンドルを import しない（全文法が 1 つのチャンクに入る）。
  正規表現エンジンの wasm も**ローカル import で**取る（外部 CDN 禁止 — CLAUDE.md §1）
- 拡張子 → 言語の対応表は `highlight.ts` に持つ。**未知の拡張子はハイライトしない**（素の本文）
- テーマはライト / ダークの 2 つを読み、`data-theme` に合わせて選ぶ。
  **Shiki の背景色は使わない** — 行の地色は追加 / 削除で決まっている。載せるのは前景色だけ
- **トークンは hunk の片側ごとにまとめて取る。** 行を 1 本ずつ渡すと、複数行にまたがる
  文字列やコメントが行ごとに切れて色が壊れる。旧側（context + removed）と
  新側（context + added）をそれぞれ 1 本のテキストに戻してトークン化し、行へ配り直す
- **トークンと語単位差分を合成する**（`dangerouslySetInnerHTML` を使わない）。
  Shiki の区切りと `wordDiff` の区切りは無関係なので、両方の境界で切り直したセグメント列を作る。
  ここは純関数でテストする
- **非同期。** 先に素の本文を描き、トークンが揃ったら差し替える。ワーカーは入れない
  （可視部分だけを描くので、1 ファイル 1 回のトークン化で足りる）

*折りたたみと仮想化*

- **1 ファイル 3000 行または 500KB 超は既定で折りたたむ**（DESIGN.md §7.2）。閾値は `ui` に置く。
  折りたたみ中は `pairHunk` / `wordSegments` / トークン化を**一切走らせない**
  （DOM を作らないだけでは足りない）
- 展開はファイルごとの一時状態。**永続化せず、別のファイルへ持ち越さない**（文字コードの上書きと同じ）
- 仮想スクロールは `@tanstack/react-virtual`（コミットリストと同じ）。
  **行の高さは実測する**（`measureElement`）— 本文を折り返すので一定でない。
  推定値は 1 行ぶんでよい（折り返す行は少数）
- **hunk の見出しも同じ行リストに入れる。** 別に描くと仮想化の対象から外れる
- 仮想化すると `Ctrl+F` が可視部分しか見つけられなくなる。承知の上（コミットリストも同じ）

*バイナリ*

- 「バイナリファイル」に**サイズの変化**を添える（DESIGN.md §7.2）。
  `cat-file -s <rev>:<パス>` を両側に対して呼ぶ。**バイナリのときだけ**呼ぶこと
- 追加 / 削除では片側が無い。`None` として扱い、**0 と混同しない**

**制約**

- **外部 CDN から文法を取らない**（外部通信禁止 — CLAUDE.md §1）。文法も wasm もバンドルに含める
- 色はテーマトークン経由、文言は `ja.ts`（CLAUDE.md §6）
- git 実行は `git::exec::run` を通す（CLAUDE.md §2）
- フロントのテストは**純関数だけ**（CLAUDE.md §8）

**受け入れ条件**

- ▸コマンド: `npm run test` — 拡張子 → 言語の対応 / トークンと語単位差分の合成
  （境界が重なる・またぐ・片方が空・トークンが無い）/ 行リストの組み立て（hunk 見出しを含む）/
  規模の計測（行数とバイト数）
- ▸コマンド: `npm run test:rust` — バイナリのサイズ取得（追加 / 削除 / 変更）
- ▸コマンド: `npm run typecheck` / `npm run check:rust`
- ▸コマンド: `npm run build` — 文法が 1 チャンクに固まっていないこと（分割されていること）
- ▸目視: ライト / ダークでハイライト色が破綻しない（語単位の強調が消えない）
- ▸目視: 5000 行以上の差分でスクロールが止まらない
- ▸目視: 大きなファイルが折りたたまれ、展開できる
- ▸目視: バイナリでサイズの変化が出る

**テスト**: `npm run test`, `npm run test:rust`, 目視

**非スコープ**: 画像差分（v1.1）／`--cc`（v1.1）／ハイライトのワーカー化
