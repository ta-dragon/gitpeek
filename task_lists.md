...
| 項目 | 内容 |
|---|---|
| 直前に完了 | **v0.2.3 の公開**（GitLab ＋ GitHub。2026-09-20。**「取り込んでいる可能性があるブランチを調べる」を速くした** — 確認用リポジトリで 15.0 秒 → 1.6 秒。**速くなったことは利用者が目視確認済み** — T-38 全体の目視はまだ）/ **v0.2.2 の公開**（GitLab ＋ GitHub。2026-09-20。**止めていないのに「止めました」と出る不具合の修正**。ここは利用者が目視確認済み）/ v0.2.1 の公開（GitLab ＋ GitHub。2026-09-19。**T-38 入り、ただし ▸目視は未了のまま**利用者の指示で公開）/ v0.2.0 の公開（2026-09-18）/ **T-37**（取り込み判定の本体。git の下限を 2.38 へ）/ T-36 / T-35（コミット検索）|
...
- **「取り込んでいる可能性があるブランチを調べる」が遅い**（2026-09-20 に利用者の指摘。「CPU をあまり
  使っていない」）。**相手ごとに並列**にし、②の `git log` を git 側で絞り、差分の指紋を覚えるようにした。
  確認用リポジトリで **15.0 秒 → 1.6 秒**、onyx の 1,896 本で **2 分 31 秒**（DESIGN.md §7.6.1 の「速さ」）
...
**目視用のリポジトリ**: `npm run demo:repo` で `~/gitpeek-demo/containment-demo` を作る（`scripts/make-demo-repo.sh`）。
**ブランチ名が期待する結果になっていて**、一覧は同じフォルダの `確認手順.md`。テストの fixture と違い、
テストを流しても消えない。枝は 80 本あるので、進み具合と「止める」もここで見られる（1 巡 16 秒ほど）。
...
| — | — | **v0.2.3 リリース**（**相手ごとに並列**にして探索を速くした（確認用リポジトリ 78 本で 15.0 秒 → 1.6 秒、onyx 1,896 本で 2 分 31 秒。DESIGN.md §7.6.1 の「速さ」）＋ 目視用リポジトリを作るスクリプト（`npm run demo:repo`）。**`--since` は履歴を辿るのをやめるので `--since-as-filter` を使う** — DESIGN.md §17.1。**速さは利用者が目視確認済み** — T-38 全体の目視はまだ）／ GitHub にも写した（SHA-256 `39a2946f…`、ログインせずに落とせることを確認）| `561a7be` |
