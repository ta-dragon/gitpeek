...
| 次にやる | **T-21 skill の読み込みと信頼モデル（Phase 8）** |
| 未解決の判断事項 | なし |

**T-21 以降は骨子**なので、着手時に詳細化そのものを最初の作業として行うこと（規約 5）。
Phase 8 は AI レビューで、T-20 → T-21 → T-22 → T-23 の順に依存している。

**T-21 で最初に効く制約は CLAUDE.md §4 の最後の項。**
「**リポジトリ内 skill (`<repo>/.gitviewer/skills/*.md`) は既定で無効**、リポジトリごとに
明示的な信頼操作を要求し、ファイル内容のハッシュが変わったら再確認する」。
**他人のリポジトリを開く用途がある以上、リポジトリ内の指示文でレビュー結果を操作されうる。**
信頼状態の置き場所は T-01 で既にある（`RepositorySettings.repo_skills`）。

**T-20 で作ったものは T-22 がそのまま使う。** `llm/client.rs` に接続と失敗の言い分けが、
`secret.rs` に API キーの出し入れが閉じている。**この 2 つの外でキーに触らないこと**
（CLAUDE.md §4）。
...
| 7 | 8 | T-20 → **T-21** → T-22 → T-23 | いまここ。T-20 は済。**T-21 以降は着手前に詳細化**（規約 5）|
| 8 | 9 | T-24 → T-25 → T-26 | |
