...
5. **骨子のままのタスクは、着手前に詳細化する。** ⑥実装内容と⑧受け入れ条件が埋まって
   ...
   **いま骨子のタスクは無い**（未着手のタスクも無い。2026-09-12 時点）。
...
2. **v1 は終わっている**（2026-09-12 に DoD が埋まった）。**決まった次の作業は無い** —
   何をするかは利用者が決める。文言の直しは「画面の名前と読んだ文」をもらって直す（下記）。
   v1.1 の候補は末尾の表
...
| 項目 | 内容 |
|---|---|
| 直前に完了 | **T-26**（**v1 の DoD が埋まった**。2026-09-12）/ **T-34**（差分が先頭の数十行で切れる不具合）|
| いまここ | **v1 完了。** 決まった次の作業は無い |
...
| T-26 | 9 | 配布ビルドと DoD 検証 — **v1 完了**。ログ 7 日（09-06〜09-12）/ 実リポジトリ 8 個 / **既存 GUI クライアントは一度も開かなかった**（2026-09-12 に利用者が回答。測り方は DESIGN.md §15.2）| |
