...
| 次にやる | **T-15 任意 2 コミット間差分 — 実装は終わり、目視確認待ち** |
| 未解決の判断事項 | なし |

**T-15 は ▸コマンド の受け入れ条件を満たしたところで止めてある。** ▸目視 が 4 項目
...
**T-15 で決めたこと（DESIGN.md §10.3 に転記済み）**

- **`A...B` は 1 つの引数**。分けて渡すとただの 2 点間差分になる
- **共通の祖先が無いときだけ git のエラーを言い換える**（`no merge base` では
  何を選んだせいか分からない）。言い換えは Rust 側に置く — git のエラーの言い換えなので
- **取得は `DiffScope` で 1 点と 2 点比較を受ける**。違いは「親を自分で決めるか」と
  「コミット詳細を取るか」の 2 つだけ
- **差分本体は一覧を作ったときの 2 点をそのまま使う**（詳細から組み直すと、読み込み中に
  一覧と差分が別の組を見る）
- **`Ctrl+クリック` は今の選択を比較元へ押し出す。** 比較中は比較元を動かさない。
  自分自身との比較にはしない。遷移は `lib/compareSelection.ts`（純関数）
- **比較元の行は塗りではなく左端の帯**。選択と同じ塗りだと起点が分からない
- **比べ方（`A...B` か）は永続化しない。** その比較かぎりの判断

**T-16 への申し送り**

- 作業ツリーとの比較も同じ経路に乗る見込み（`DiffScope` に種類を足す）。
  ただし作業ツリーはリビジョンではないので、`changed_files` の引数の形は再検討が要る
