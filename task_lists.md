...
| 未解決の判断事項 | 判定用リポジトリが 1 種類足りない（下記） |
...
**判定用リポジトリが足りない。** T-08 は 5 種類を求めているが、登録済みで枝分かれがあるのは
onyx だけ（他の 3 個はマージ 0 件の一直線、linux は非対象）。
**「OSS の中規模（1,000〜5,000 コミット、マージあり）」を 1 個用意してから合否を出すこと。**
...
- **手元のリポジトリは 5 種類に足りていない。** 登録済みは gitviewer(24) /
  AerialSoldierMaker(22) / save_temperature(10) / linux(148 万・非対象) / onyx(20,285) の 5 個で、
  **前 3 つはマージが 1 件も無い一直線**、linux は DESIGN.md §4.1 の非対象。
  実質 onyx 1 個でしか枝分かれを見られていない。**「OSS の中規模（1,000〜5,000 コミット）」が
  空いているので、判定の前にユーザーに用意してもらう。**
