...
| 項目 | 内容 |
|---|---|
| 直前に完了 | **T-34**（差分が先頭の数十行しか描かれない不具合。**v0.1.3 としてはまだ出していない**）|
| いまここ | **T-26 の実運用**（**2026-09-12 に 7 日ぶん / 8 リポジトリが揃った**。▸目視 1 問の回答待ち）|
...
決めたことと理由は DESIGN.md §6.4。**実運用はこの版で続ける**（T-26 の DoD は「既存 GUI を
開かずに済んだか」なので、途中で版が上がっても数え直しにはならない）。

**作ってみて分かったこと（落とし穴・割り切り）は DESIGN.md §17。**

**T-26 に残るは ▸目視 1 問だけ。** ログは 2026-09-12 に 7 日ぶん / 8 リポジトリ揃い、
▸コマンドの条件は満たした。▸目視 1 問（既存 GUI クライアントを一度も開かずに済んだか）へ
答えてもらえば v1 の DoD が埋まる。
...
**2026-09-12 時点で実行済み: 7 日ぶん（09-06〜09-12）/ 8 リポジトリ。▸コマンドの 2 条件（7 日・5 個以上）は満たした。**
**09-11 のログには、T-34 の調査で起動した開発版の行も混じっている。** 同じ日に利用者の
日常使い（v0.1.2）も動いていたので、日数の数え方は変わらない。リポジトリも既存の 8 個のうちの 1 つ。
リポジトリ数は既に足りている（`drogon` / `dycora` / `linux` / `onyx` / `vite` /
`AerialSoldierMaker` / `gitviewer` / `save_temperature`）。**残るは ▸目視 1 問だけ。**
...
| T-34 | — | 差分が先頭の数十行しか描かれない不具合（1 本の ref を外枠と内側に付けていた。**テストでは固定できない**。DESIGN.md §17.1）／ `Enter` のフォーカス移動は効いていないように見えるが、利用者が求めないので直さない（§17.2）| |
