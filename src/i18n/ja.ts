...
export const ja = {
  ...
  /**
   * レビュー skill（T-21）。**設定モーダルのタブ 1 つ。**
   *
   * 文言は「何が起きるか」で書く（CLAUDE.md §6）。とくに信頼の話は、
   * **なぜ既定で無効なのか**が読めないと、ただ手間が増えたようにしか見えない。
   */
  skills: {
    tab: "レビュー観点",
    title: "レビュー観点（skill）",
    lead: "AI レビューで何をどう見るかを決めるファイルです。内蔵のものが 1 つあり、増やすこともできます。",
    ...
    originNote: {
      builtIn: "Givsoner に同梱されています。編集はできません。",
      global: (dir: string) => `${dir} に置いてあります。`,
      repository: ".gitviewer\\skills\\ に置かれています。",
    },

    // --- 状態 ---------------------------------------------------------
    stateReady: "使います",
    stateUntrusted: "使いません",
    stateRecheck: "止めています",
    stateUnreadable: "読めません",
    // **効かない理由は必ず出す。** 消さない代わりにここで説明する。
    stateNote: {
      untrusted:
        "このリポジトリの中にあるファイルなので、確認するまで使いません。中身を読んでから「このリポジトリの観点を使う」を押してください。",
      recheck:
        "前に確認したときから中身が変わっています。もう一度読んでから使うか決めてください。それまでは使いません。",
    },
    // 同名で押しのけられた。**どちらが効いているのか必ず出す。**
    shadowed: (winner: string) => `同じ名前の「${winner}」のほうを使っています。`,

    // --- 一覧 ---------------------------------------------------------
    counts: (usable: number) => `${usable} 件を使います`,
    countsHeld: (held: number) => `${held} 件を止めています`,
    countsUnreadable: (n: number) => `${n} 件は読めません`,
    always: "いつも使う",
    onlyWhen: (globs: string) => `${globs} に当てはまる変更があるときだけ使う`,
    offByDefault: "既定では使いません（レビューのときに選べば使えます）。",
    openFolder: "置き場所を開く",

    // --- 信頼 ---------------------------------------------------------
    //
    // **なぜ既定で無効なのかを最初に書く。** 手間の理由が読めないと、
    // 「とりあえず押す」ボタンになってしまう。
    trustWhy:
    ...
    untrust: "使うのをやめる",
    trustNote: {
      noRepository: "リポジトリを開くと、そのリポジトリの観点をここで確認できます。",
      noSkills: "このリポジトリには観点のファイルが置かれていません。",
      untrusted: "中身を読んでから決められます。押すと本文を表示します。",
      recheck: "前に確認したときから中身が変わっています。押すと、変わったところを含めて表示します。",
      trusted: "確認済みです。中身が変わったら、また確認をお願いします。",
    },

    // 確認画面。**読ませずに信頼させない。**
    reviewTitle: "このリポジトリの観点を読む",
    reviewLead:
      "これから使う観点の全文です。レビュー結果を左右するので、読んでから決めてください。",
    reviewChanged: (files: string) => `中身が変わったファイル: ${files}`,
    reviewAdded: (files: string) => `前の確認のあとに増えたファイル: ${files}`,
    reviewUnreadableNote:
      "読めないファイルも一覧に出しています。心当たりが無いファイルがあれば、使うのをやめてください。",
    reviewAccept: "読みました。使います",
    reviewCancel: "やめる",
    untrustConfirm:
      "このリポジトリの観点を使うのをやめます。次に使うときは、もう一度中身を確認します。",
