...
ブランチが取り込まれているかを調べるときだけ `merge-tree` で仮にマージしますが、書き込むのは
**リポジトリの外の一時フォルダだけ**で、終わったら消します（リポジトリには何も書きません）。

## できること

- [x] **履歴グラフ** — 全コミットをレーン付きで一度に描画（数万コミット規模まで）
- [x] **ブランチ / タグのツリー** — 表示の ON/OFF、ahead / behind の表示、**更新日で絞り込む**、
  **チェックしたブランチの一覧（名前と最終コミット時刻）を CSV に保存**
- [x] **ブランチが取り込まれているか** — squash マージや cherry-pick で中身が入っていて、元のブランチだけが
  残っているものに印を付けます（普通のマージ・途中まで入っている・入った後に書き換えられた、も見分けます）。
  印を押すと、まとめて入れたコミットへ移動します。右クリックからは、そのブランチを取り込んでいる
  可能性があるブランチを全部調べて並べられます（途中で止められます）
- [x] **差分** — 構文色、左右に並べる表示、語単位の差分、差分の中の検索
- [x] **コミットを探す** — メッセージ・作者から探し、当たった行に印を付けて前後に辿る。
  `message:` / `author:` / `code:` で絞り込めます（`code:` はコードの中身から探すので時間がかかり、
  目安の時間を出して途中で止められます）
...
|---|---|
| OS | Windows 10 / 11 |
| git | **2.38 以上**で、PATH が通っていること（通っていなければ、初回起動時の画面でフルパスを指定できます） |
